...
此文档定义 AI 生成的 Product App 代码中可用的全部 API，供 Agent 工具 system prompt 或调试时参考。

> **实际全局对象为 `window.app`**，以下各节均基于 `window.app`。

## 能力边界（先看这一节）

Product App **能且只能**用以下 API，没有任何"通用 Sparo OS 后端通道"。生成代码前请先确认你需要的能力在表内：
...
1. 能用裸命令行解决就用 `app.shell.exec`（如 git → 在 `permissions.shell.allow` 加 `"git"`，参考 `surface_component/Demo/git-graph`）；
...
Product App 中所有与宿主通信的 API 均通过 `window.app` 暴露。

### 基本属性

```javascript
app.appId        // string — 当前 Product App 的 ID
...
返回当前 Product App 权限范围内可用的模型列表（不含 API Key 等敏感信息）。
...
当 Product App 需要创建/管理一个真正的 Sparo OS Agentic 会话、把 prompt 交给 Sparo OS 执行、接收工具事件/错误/完成事件，并允许用户跳转到原生 Flow Chat 查看时，使用 `app.agentic.*`。
...
const session = await app.agentic.createSession({
  ...
  agentType: 'agentic', // 'agentic' | 'Plan' | 'AppStudio' 等权限允许的模式
  model: 'auto',
});
...
宿主只会把当前 Product App 拥有的 Agentic session 事件转发进 iframe。
...
```json
{
  "permissions": {
    "agentic": {
      "enabled": true,
      "allowed_agents": ["agentic", "Plan", "AppStudio"],
...
- `allow_workspace` 默认保持 `false`。只有用户明确要求 Product App 绑定/读取当前工作区时才开启，并写清 `permission_rationale`。
- `allow_tools` 为 `true` 时仍由 Sparo OS safe mode 和工具确认机制保护，不要在 Product App 中静默批准高风险工具。
...
参考实现：`bundles/surface-components/spark-board/ui.js`、`bundles/surface-components/ppt-live/ui.js`。
...
宿主会把这些框架原语直接路由到 Rust 实现（`bitfun_core::surface_component::host_dispatch`），完全不需要 Bun/Node 运行时；权限策略与 Worker 路径共用同一份 `resolve_policy`，行为完全等价。在这种模式下：
...
- `{appdata}` — `{user_data_dir}/surface_components/{app_id}/data/`，始终可读写
