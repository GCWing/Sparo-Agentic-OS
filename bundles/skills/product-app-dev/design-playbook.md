# Product App 设计与生成 Playbook

> 这份 Playbook 用于**生成一个新的 Product App**。AI 在使用 `CreateProductApp` 工具创建骨架后，必须遵循本指南完成实现，以避免典型的 "AI 味" 产出。
>
...
1. 用户提供的截图 / 品牌资料 / 现成代码
2. `surface_component/Demo/` 与 `bundles/surface-components/` 中**最贴近形态**的内置应用 —  — 直接 `ls` + `Read` 拿到它的 `style.css`、`index.html`，识别它的视觉语言（间距、圆角、卡片密度、配色）
...
| 一上来就写 1500 行 ui.js            | 早提交早预览；功能成型后再分模块（参考 `surface_component/Demo/git-graph` 的拆分）                                                     |
...
> 灵感源：在最终用户那里，"一份代码服务多种偏好"才是 Product App 的天然优势。
...
## 七、与 Sparo OS 工具型 Product App 的契合度
...
- `bundles/surface-components/spark-board/` — 工具型工作台、主题切换 + i18n + 持久化范例
- `bundles/surface-components/ppt-live/` — 复杂应用拆模块、AI 辅助与导出流程范例
- `surface_component/Demo/git-graph/` — 复杂应用拆模块的范例（`ui/components`, `ui/panels`, `ui/services`）
- `surface_component/Demo/icon-design-system/` — 设计系统型应用范例
