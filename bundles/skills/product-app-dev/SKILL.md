...
description: Develops, maintains, and generates Sparo OS Product Apps. Use for Product App framework code, AppStudio, CreateProductApp, window.app APIs, app.fs/app.shell/app.storage/app.net/app.ai/app.agentic, runtime debugging, or the underlying Product App runtime.
---

# Sparo OS Product App V2 开发指南

> **本 Skill 服务两类工作**：
>
> - **维护框架本身** → 阅读下方"代码架构 / Bridge / 权限模型 / window.app API"等章节。
> - **生成一个新的 Product App** → 先遵循 Studio prompt 的默认策略；本文件只提供 API、权限、视觉硬约束。需要深入视觉 polish 时再读 `[design-playbook.md](design-playbook.md)`。详细 API 见 `[api-reference.md](api-reference.md)`。
...
1. 任意 Agent 用 `Skill('product-app-dev')` 加载（手动调用，框架维护或生成新 Product App）。
2. **AppStudio Mode 自动调用**：每个 Studio 会话首轮强制加载本 `SKILL.md`。Studio 的工作循环（Intake / Anchor / Scaffold / Skeleton / Loop / Polish / Review）已写入它的 prompt，本文档不重复。本文档只提供 Anchor 阶段的视觉锚定上下文与 Polish 阶段的 QA Checklist。


| 场景                  | 必读                                | 按需 Read                                    |
| ------------------- | --------------------------------- | ------------------------------------------ |
| Studio 生成新 Product App | `SKILL.md` | `design-playbook.md`（深度视觉 polish 时）、`api-reference.md`（写 `worker.js` / 自定义权限时） |
...
按"工具型 → 展示型 → 设计系统型"递进选择最贴近的形态后 Read 其 `style.css` 顶部 Design System 注释。以下路径只保证在开发仓库中存在；二进制发布版应优先依赖内置 `product-app-dev` Skill 的 compact baseline 和已安装 Product App，不要假设 Demo 目录存在。

- **内置（克制工具型）**：`bundles/surface-components/`
  - `gomoku/` — 交互 + 主题切换 + 持久化
  - `divination/` — 仪式感配色与卡片
- **Demo（更激进 / 复杂模块化）**：`surface_component/Demo/`
...
## 生成新 Product App 必读（速查）

> 完整指南见 `[design-playbook.md](design-playbook.md)`。这里是**不可妥协**的硬约束，AI 在用 `CreateProductApp` 工具创建骨架后**必须**遵守。
...
2. **找设计上下文**：开发仓库中可先读 `surface_component/Demo/` 与 `bundles/surface-components/` 中**最贴近形态**的应用，复刻它的视觉语言（间距 / 圆角 / 卡片密度 / motif）。如果这些路径不存在（例如二进制发布版），不要搜索用户工作区凑参考，直接使用 packaged `product-app-dev` 的设计 baseline。
...
- **优先使用运行态 UI Kit**：常规 Button / Card / Input / Badge / Alert / Empty / Stack / Toolbar 优先使用 `app.ui`，它随 Product App 编译产物注入，不需要 import，也不依赖主应用开发态别名。
- 一个颜色占视觉权重 60-70%（dominant），1-2 个 supporting，1 个 accent —  — **禁止给所有色块同等权重**。
- 字号：标题 18-22px / Section 14-15px / 正文 13-14px / Caption 11-12px。

### Tweaks 变体（推荐做法）

对外观/密度/字号/布局的多种合理选择，做成运行时可切换、写入 `app.storage('tweaks')`、右下角浮动小面板"Tweaks" —  — 一份代码服务多种偏好是 Product App 的天然优势。详细约定见 playbook §4。
...
绝大多数 Sparo OS Product App 是**工具型** —  — 信息密集、操作短、配色冷静，仿照 `gomoku` / `divination` / `git-graph` 的克制感。只有用户明确要"对外展示 / 灵感型 / 作品集"时才放飞视觉。
...
Product App 使用 **标准 Web API + window.app**：UI 侧为 ESM 模块（`ui.js`），后端逻辑在独立 JS Worker 进程（Bun 优先 / Node 回退）中执行。Rust 负责进程管理、权限策略和 Tauri 独占 API；Bridge 统一通过 **window.app** Runtime Adapter 暴露宿主能力。
...
```
src/crates/core/src/surface_component/
├── types.rs               # SurfaceComponentSource (ui_js/worker_js/esm_dependencies/npm_dependencies), NodePermissions
...
```
src/apps/desktop/src/api/surface_component_api.rs
```

- 应用管理: `list_surface_components`, `get_surface_component`, `create_surface_component`, `update_surface_component`, `delete_surface_component`
- 存储/授权: `get/set_surface_component_storage`, `grant_surface_component_workspace`, `grant_surface_component_path`
- 版本: `get_surface_component_versions`, `rollback_surface_component`
- Worker/Runtime: `surface_component_runtime_status`, `surface_component_worker_call`, `surface_component_host_call`, `surface_component_worker_stop`, `surface_component_install_deps`, `surface_component_recompile`
- 对话框由前端 Bridge 用 Tauri dialog 插件处理，部分封装为 `surface_component_dialog_*` 等

### Agent 工具

```
src/crates/core/src/agentic/tools/implementations/
└── surface_component_init_tool.rs   # CreateProductApp — 创建骨架目录供 AI 用通用文件工具编辑
```

注册在 `registry.rs` 的 `register_all_tools()` 中。AI 后续用 Read/Edit/Write 等通用文件工具编辑 Product App 源文件。

### 前端

```
src/web-ui/src/app/scenes/apps/surface-component/
├── surfaceComponentStore.ts
├── components/ SurfaceComponentCard, SurfaceComponentRunner (iframe 带 data-app-id)
├── hooks/
│   ├── useSurfaceComponentBridge.ts        # worker.call → workerCall() + dialog.open/save/message
│   └── useSurfaceComponentCatalogSync.ts   # 列表与运行态同步
└── surfaceComponentIcons.tsx 等

src/web-ui/src/app/scenes/apps/SurfaceComponentScene.tsx / SurfaceComponentScene.scss
src/web-ui/src/infrastructure/api/service-api/SurfaceComponentAPI.ts
src/web-ui/src/flow_chat/tool-cards/CreateProductAppToolDisplay.tsx   # CreateProductAppDisplay，工具名 CreateProductApp
```
...
SurfaceComponentPermissions { fs?, shell?, net?, node?, ai?, agentic? }  // node 替代 env/compute，AI/Agentic 走宿主桥接
```
...
```
iframe 内 window.app.call(method, params) → useSurfaceComponentBridge 监听
  ├─ 框架原语 (fs.* / shell.* / os.* / net.*)：
  │   ├─ node.enabled = false → surfaceComponentAPI.hostCall → Tauri invoke('surface_component_host_call')
  │   │ → bitfun_core::surface_component::host_dispatch（纯 Rust，无需 Bun/Node）
  │   └─ node.enabled = true → surfaceComponentAPI.workerCall → Tauri invoke('surface_component_worker_call')
  │ → JsWorkerPool（保留旧路径，允许 worker.js 覆写 fs/shell 等）
  ├─ 自定义方法：始终走 worker.call → JsWorkerPool（要求 node.enabled = true 且 worker.js 导出）
  ├─ storage.* (node.enabled = false 时)：直接走 get/set_surface_component_storage 命令
  ├─ ai.*：走 surface_component_ai_* 命令，复用宿主 AIClient
  └─ agentic.*：走 surface_component_agentic_* 命令，创建/驱动宿主管理的 Sparo OS Agentic 会话

dialog.open / dialog.save / dialog.message → postMessage → useSurfaceComponentBridge 直接调 @tauri-apps/plugin-dialog
```
...
Product App 运行时**只暴露下列能力**，没有任何任意调用内部服务的"通用 Sparo OS 后端通道"。设计 / 生成新小应用前请先比对，能力不在表内的需求请走相应替代方案。Agentic 会话只能通过表内的 `app.agentic.*` 管理，**不要假设有 `app.sparo.*` / `app.workspace.*` / `app.git.*` / `app.session.*` 之类的接口存在。**
...
每个编译后的 Product App 都会注入一个小型运行态 UI Kit：`window.app.ui`。它不是主应用 React 组件库的开发态直连，而是为 sandbox iframe 准备的白名单子集：用原生 DOM helper + 与组件库对齐的 class contract，避免 Product App 依赖 Vite alias、React context 或开发服务器。
...
生成常规工具型 Product App 时，优先用 `app.ui` 搭骨架，再写业务样式。若必须手写 HTML，也优先复用这些运行态 class：`btn`、`v-card`、`bitfun-input-wrapper`、`badge`、`alert`、`bfui-stack`。
...
- Session / AgenticSystem 的任意内部 API（只能用 `app.agentic.*` 创建/恢复 Product App 自己拥有的会话、发送 prompt、接收事件、确认/拒绝工具、跳转查看）
...
Product App UI 内通过 **window.app** 访问：
...
Product App 在 iframe 中运行时自动与主应用主题同步，避免界面风格与主应用差距过大。
...
宿主会将主应用主题映射为以下 CSS 变量并注入 iframe 的 `:root`。在 Product App 的 CSS 中建议用 `var(--sparo-*, <fallback>)` 引用，以便在 Sparo OS 内与主应用一致，导出为独立应用时 fallback 生效。

审阅时必须把下列清单当作唯一有效的宿主主题变量白名单。任何不在清单内的 `--sparo-*` 引用都是无效变量；任何 `--theme-*` / `--color-*` / `--surface-*` 之类主题名只有在 Product App 自己的 `:root` 明确定义后才能使用。
...
Product App 运行在 V2 之后内置 i18n 支持，开发者**必须**为多语言用户考虑两类文案：
...
宿主不强制要求该写法，但推荐 Product App 内部统一约定：
...
参考 `bundles/surface-components/spark-board/ui.js` 等内置应用的 `I18N` 表 + `applyStaticI18n()` + `app.onLocaleChange` 三件套即可复用。
...
## 内置小应用（bundles/surface-components/*）维护规范
内置小应用放在 `bundles/surface-components/<app>/`，每个包用 `bundle.json` 声明稳定 `id` 和 reseed `version`。`src/crates/core/src/surface_component/builtin/mod.rs` 会以 `include_dir!` 打包整个 `bundles/surface-components` 目录；首次启动 / 升级时由 `seed_builtin_surface_components()` 把资源写入用户的 `surface_components/<app_id>/`，并在该目录下写入 `.builtin-version` 标记文件。

**只有当 bundled `version` > on-disk 标记时才会重新 seed**，否则启动时会跳过，用户看到的还是旧版本。

### 修改流程（强制）

凡是修改了 `bundles/surface-components/<app>/` 下任何文件（`index.html` / `style.css` / `ui.js` / `worker.js` / `meta.json`），**都必须**同步把该包 `bundle.json` 的 `version: N` 改为 `N + 1`。
...
- 已经体验过该小应用的用户（本地有 `.builtin-version` 标记）不会收到新版本，无法验证设计 / 修复
- QA / Release 看到的还是旧文件，会误判“代码已合但效果没出来”

### 自检清单

- 改完 `bundles/surface-components/<app>/` 任何文件
- `bundle.json` 中对应 `version` 已 +1
- 本地清掉用户数据目录中 `surface_components/<app_id>/.builtin-version` 或直接删整目录，再启动验证 reseed 生效
- `meta.json` 中的 `version` 字段（用户可见的元数据版本）按需同步（与 reseed 无关，但用于展示）

### 提示

- `meta.json` 里的 `version` 是给用户看的版本号，不驱动 reseed
- 真正驱动 reseed 的是 `bundle.json` 中的 `version` 字段（u32）
- 二者最好语义一致：资源有重大更新时同步 bump，便于排查
## 开发约定

### 新增 Agent 工具

当前仅 **CreateProductApp**。若扩展：

1. `implementations/surface_component_xxx_tool.rs`（或他名）实现 `Tool`
...
后端 `surface-component-created` / `surface-component-updated` / `surface-component-deleted` / `surface-component-worker-`*，前端 `useSurfaceComponentCatalogSync` 统一监听并刷新 store。
