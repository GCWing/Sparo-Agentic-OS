...
import {
  bindManuscriptController,
  captureManuscriptBase,
  commitDeckStateToManuscript,
  commitPlanToManuscript,
  commitStylePresetToManuscript,
  normalizeSlideMarkerId,
  resetManuscriptForNewDeck,
} from './src/manuscript-editor.js';
...
let visualStateReady = false;
let latestObservedManuscriptDocument = null;
let localManuscriptCommitDepth = 0;
...
async function restoreHistory(id) {
  ...
  localManuscriptCommitDepth += 1;
  try {
    const restoredState = ensureState(clone(item.state));
    const manuscriptBase = await captureManuscriptBase();
    const committedManuscript = await commitDeckStateToManuscript({
      deckState: restoredState,
      baseDocument: manuscriptBase,
      reason: 'history-restore',
    });
    state = restoredState;
    markVisualsAligned(committedManuscript);
    ...
    runtime().log?.warn?.('Failed to restore PPT Live history with its manuscript', { error: String(error) });
    setStatus(error instanceof Error ? error.message : String(error));
  } finally {
    localManuscriptCommitDepth -= 1;
    if (localManuscriptCommitDepth === 0) await reconcileObservedManuscript();
  }
}
...
function setStatus(message) {
  const node = $('statusLine') || $('topProgressText');
  if (node) node.textContent = message;
}

function setExportStatus(message) {
  const node = $('exportStatus') || $('topProgressText');
  if (node) node.textContent = message;
}
...
function rerender() {
  ...
  renderVisualAlignment();
  renderHistory();
}

function renderVisualAlignment() {
  const node = $('deckMeta');
  if (!node) return;
  const base = t('slidesMeta', { count: state.slides.length });
  const binding = state.manuscriptBinding || {};
  const parts = [base];
  if (binding.status === 'stale') {
    parts.push(t('manuscriptVisualStale', { revision: binding.latestRevision || '—' }));
  } else if (binding.status === 'visual_ahead') {
    parts.push(t('manuscriptVisualAhead'));
  } else if (binding.status === 'conflict') {
    parts.push(t('manuscriptVisualConflict'));
  } else if (binding.status === 'aligned' && binding.basedOnRevision) {
    parts.push(t('manuscriptVisualAligned', { revision: binding.basedOnRevision }));
  }
  const missing = Array.isArray(state.visualCoverage?.missingSlideNumbers)
    ? state.visualCoverage.missingSlideNumbers
    : [];
  if (missing.length) {
    parts.push(t('manuscriptVisualPartial', { missing: missing.join(', ') }));
  }
  node.textContent = parts.join(' · ');
}

function alignedManuscriptBinding(documentValue, status = 'aligned') {
  return {
    basedOnRevision: Number(documentValue?.revision) || 0,
    basedOnContentHash: String(documentValue?.contentHash || ''),
    latestRevision: Number(documentValue?.revision) || 0,
    latestContentHash: String(documentValue?.contentHash || ''),
    status,
  };
}

function markVisualsAligned(documentValue, status = 'aligned') {
  const binding = alignedManuscriptBinding(documentValue, status);
  state.manuscriptBinding = binding;
  state.slides = state.slides.map((slide) => ({
    ...slide,
    manuscriptBinding: alignedManuscriptBinding(documentValue),
  }));
}

function markVisualAhead(slideIds = null) {
  const current = state.manuscriptBinding || {};
  if (!current.basedOnContentHash && !current.latestContentHash) return;
  const targetIds = Array.isArray(slideIds) ? new Set(slideIds.filter(Boolean)) : null;
  const nextDeckStatus = ['stale', 'conflict'].includes(current.status) ? 'conflict' : 'visual_ahead';
  state.manuscriptBinding = { ...current, status: nextDeckStatus };
  state.slides = state.slides.map((slide) => {
    if (targetIds && !targetIds.has(slide.id)) return slide;
    const slideBinding = slide.manuscriptBinding || current;
    const nextStatus = ['stale', 'conflict'].includes(slideBinding.status) ? 'conflict' : 'visual_ahead';
    return {
      ...slide,
      manuscriptBinding: {
        ...slideBinding,
...
function observeManuscriptDocument(documentValue) {
  const contentHash = String(documentValue?.contentHash || '');
  const revision = Number(documentValue?.revision) || 0;
  if (!contentHash || !revision) return false;
  const observedRevision = Number(latestObservedManuscriptDocument?.revision) || 0;
  if (revision < observedRevision) return false;
  if (
    revision === observedRevision
    && latestObservedManuscriptDocument?.contentHash
    && latestObservedManuscriptDocument.contentHash !== contentHash
  ) {
    runtime().log?.warn?.('Ignored inconsistent PPT manuscript event for an observed revision', {
      revision,
    });
    return false;
  }
  latestObservedManuscriptDocument = documentValue;
  return true;
}

function applyObservedManuscriptDocument(documentValue) {
  const contentHash = String(documentValue?.contentHash || '');
  const revision = Number(documentValue?.revision) || 0;
  if (!contentHash || !revision) return false;
  const current = state.manuscriptBinding || {};
  if (revision < (Number(current.latestRevision) || 0)) return false;
  if (current.basedOnContentHash === contentHash) {
    state.manuscriptBinding = alignedManuscriptBinding(
      documentValue,
      current.status && current.status !== 'unbound' ? current.status : 'aligned',
    );
  } else {
    const status = state.slides.length === 0
      ? 'aligned'
      : ['visual_ahead', 'conflict'].includes(current.status) ? 'conflict' : 'stale';
    state.manuscriptBinding = {
      ...current,
  ...
  state.slides = state.slides.map((slide) => ({
    ...slide,
    manuscriptBinding: {
      ...(slide.manuscriptBinding || {}),
      latestRevision: revision,
      latestContentHash: contentHash,
      status: slide.manuscriptBinding?.basedOnContentHash === contentHash
        ? (['visual_ahead', 'conflict'].includes(slide.manuscriptBinding?.status)
          ? slide.manuscriptBinding.status
          : 'aligned')
        : ['visual_ahead', 'conflict'].includes(slide.manuscriptBinding?.status) ? 'conflict' : 'stale',
    },
  }));
  return true;
}

async function reconcileObservedManuscript({ persistChanges = true } = {}) {
  if (!visualStateReady || !latestObservedManuscriptDocument) return false;
  const changed = applyObservedManuscriptDocument(latestObservedManuscriptDocument);
  if (!changed) return false;
  rerender();
  if (persistChanges && !backendRunInFlight) await persist(true);
  return true;
}

function handleVisualManuscriptCommitted(documentValue) {
  if (!observeManuscriptDocument(documentValue)) return;
  if (!visualStateReady || backendRunInFlight || localManuscriptCommitDepth > 0) return;
  void reconcileObservedManuscript();
}
...
function backendErrorDetail(error) {
  ...
  return compactText(raw
    ...
    .replace(/^product_app_runtime_backend_call:\s*/i, '')
...
async function runPptLiveBackend(operation, instruction, options = {}) {
  ...
  try {
    ...
    await runLegacyBackendWithRetries(operation, instruction, deckEpoch);
  } finally {
    backendRunInFlight = false;
    await reconcileObservedManuscript();
  }
}

async function runLegacyBackendWithRetries(operation, instruction, runEpoch) {
  let lastError = null;
...
async function executeBackendTurn(requestInput, hooks = {}) {
  const host = runtime();
  if (!host.backend?.onEvent) throw new Error('PPT Live backend events are unavailable');
  ...
  const actionRunId = `ppt-live-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
  const earlyEvents = [];
  let earlyListenerActive = true;
  const earlyListener = (event) => {
    const eventTurnId = String(event?.turnId || event?.actionRunId || '');
    if (eventTurnId === actionRunId) earlyEvents.push(event);
  };
  host.backend.onEvent(earlyListener);
  cleanup.push(() => {
    if (earlyListenerActive) host.backend.offEvent?.(earlyListener);
  });
  ...
  try {
    ...
    const waitForResult = new Promise((resolve, reject) => {
      ...
      host.backend.offEvent?.(earlyListener);
      earlyListenerActive = false;
      earlyEvents.splice(0).forEach(listener);
    ...
    const shouldStop = () => isDeckEpochStale(runEpoch);
    const streamed = await waitForBackendResultOrPersistedText(
      waitForResult,
      sessionId,
      turnId,
      activity,
    ...
    if (isDeckEpochStale(runEpoch)) throw new Error('Generation stopped');
    const finalText = await resolveBackendTurnText(
      sessionId,
      turnId,
      streamedText,
      streamedThinking,
...
function manuscriptHasAuthoredPlan(documentValue) {
  const content = String(documentValue?.content || '');
  if (/^<!--\s*ppt:slide\b/m.test(content)) return true;
  const title = content.match(/^#\s+(.+)$/m)?.[1]?.trim() || '';
  return Boolean(title && title.toLowerCase() !== 'untitled deck');
}

function canonicalManuscriptContext(documentValue) {
  return {
    documentId: String(documentValue?.documentId || 'manuscript'),
    revision: Number(documentValue?.revision) || 0,
    contentHash: String(documentValue?.contentHash || ''),
    content: String(documentValue?.content || ''),
    authored: manuscriptHasAuthoredPlan(documentValue),
  };
}

function manuscriptSlideIds(documentValue) {
  return [...String(documentValue?.content || '').matchAll(
    /^<!--\s*ppt:slide\s+id="([A-Za-z0-9._-]+)"[^>]*-->\s*$/gm,
  )].map((match) => match[1]);
}
...
async function runPptLiveBackendAttempt(operation, instruction, attempt = 1) {
  ...
  try {
    const manuscriptBase = await captureManuscriptBase();
    const stateBeforeApply = clone(state);
    const payload = await executeBackendTurn({
      ...buildBackendRequestBase(operation, instruction),
      manuscript: canonicalManuscriptContext(manuscriptBase),
      title: state.title,
      outline: clone(state.outline),
    ...
    try {
      const mutation = applyDeckPayload(payload);
      const committedManuscript = await commitDeckStateToManuscript({
        deckState: state,
        baseDocument: manuscriptBase,
      ...
      state = ensureState(stateBeforeApply);
      throw error;
    }
...
async function runWithConcurrencyLimit(tasks, limit, shouldStop = () => false) {
...
async function runStagedPlanPhase(operation, instruction, runEpoch, manuscriptBase) {
  let lastError = null;
  for (let attempt = 1; attempt <= PPT_BACKEND_MAX_ATTEMPTS; attempt += 1) {
    if (isDeckEpochStale(runEpoch)) throw new Error('Generation stopped');
    try {
...
async function runStagedSlideBatch({ operation, instruction, planContext, batch, batchNumber, checkpoint, onSlideCheckpoint, runEpoch }) {
...
async function runStagedDeckGeneration(operation, instruction) {
  ...
  try {
    ...
    // Capture the canonical text revision before the long-running plan call.
    // A concurrent editor commit then fails CAS instead of being overwritten.
    const manuscriptBase = await captureManuscriptBase();
    const { payload: planPayload, slidePlans } = await runStagedPlanPhase(
      operation,
      instruction,
      runEpoch,
      manuscriptBase,
    ...
    const canonicalSlideIds = manuscriptSlideIds(manuscriptBase);
    const normalizedPlans = slidePlans.map((plan, index) => {
      const slideNumber = slidePlanNumber(plan, index + 1);
      return {
        ...plan,
        slideNumber,
        slideId: normalizeSlideMarkerId(
          plan.slideId || plan.id || canonicalSlideIds[index],
          uid(`slide-spec-${slideNumber}`),
        ),
      };
    });
    const committedManuscript = manuscriptHasAuthoredPlan(manuscriptBase)
      ? manuscriptBase
      : await commitPlanToManuscript({
        plan: planPayload,
    ...
    // Phase 2 consumes the committed text plan. Visual workers may run in
    // parallel, but they must not rewrite the canonical manuscript.
    const planContext = {
    ...
    const results = await runWithConcurrencyLimit(
      ...
      () => isDeckEpochStale(runEpoch),
    );
    if (isDeckEpochStale(runEpoch)) throw new Error('Generation stopped');
    const latestManuscript = await captureManuscriptBase();
    if (
      latestManuscript.revision !== committedManuscript.revision ||
      latestManuscript.contentHash !== committedManuscript.contentHash
    ) {
      throw new Error('ppt.manuscript.revision_conflict: the text draft changed while visual slides were rendering');
    }

    // Phase 3: assemble.
    const failures = results.filter((result) => result.status === 'rejected');
    const slidePlanByNumber = new Map(normalizedPlans.map((plan) => [plan.slideNumber, plan]));
    const readySlides = [...checkpoint.entries()]
      .sort((a, b) => a[0] - b[0])
      .map(([number, slide]) => ({
        ...slide,
        // Models are not trusted to echo identity fields. Re-attach the
        // canonical slide ID from the committed manuscript by slide number.
        id: slidePlanByNumber.get(number)?.slideId || slide.id,
        slideId: slidePlanByNumber.get(number)?.slideId || slide.slideId,
      }));
    ...
    const missingNumbers = failures.length
      ? normalizedPlans
...
async function stopAllBackendRuns(fromTimeout = false, options = {}) {
  deckEpoch += 1;
  const hadRuns = backendRuns.length > 0 || backendRunInFlight || state.generation.active;
  await cancelTrackedBackendRuns();
  state.generation.active = false;
...
function applyDeckPayload(payload) {
  const patchMutation = applyDeckPatchPayload(payload);
  if (patchMutation) {
  ...
  return {
    changedSlideIds: state.slides.map((slide) => slide.id),
    deletedSlideIds: [],
    replaceAllSlides: true,
    updateManagedHeader: true,
  };
}
...
async function waitForBackendResultOrPersistedText(
  waitForResult,
  sessionId,
  turnId,
  activity = null,
...
async function resolveBackendTurnText(
  sessionId,
  turnId,
  streamedText,
  streamedThinking = '',
...
function replaceActiveSlide(nextSlide) {
  ...
  markVisualAhead([state.slides[index].id]);
}
...
function syncStylePropertyControls() {
  ...
  if (select) {
    select.value = state.style.stylePreset || DEFAULT_STYLE_PRESET;
    refreshFlatSelect(select);
  }
}

function setManagedStylePreset(selected) {
  ...
  syncStylePropertyControls();
  return true;
}

async function changeManagedStylePreset(selected) {
  const previousStyle = clone(state.style);
  if (!setManagedStylePreset(selected)) return;

  if (!hasUsableDeckForRevision()) {
    localManuscriptCommitDepth += 1;
    try {
      const manuscriptBase = await captureManuscriptBase();
      const committedManuscript = await commitStylePresetToManuscript({
        baseDocument: manuscriptBase,
        stylePreset: selected,
      });
      updateBriefFromInputs({ includeTopic: true });
      state.slides = state.slides.map((slide, index) => normalizeSlide(
      ...
      syncStylePropertyControls();
      runtime().log?.warn?.('Failed to update the managed PPT manuscript style', {
        stylePreset: selected,
        error: String(error),
      });
      setStatus(error instanceof Error ? error.message : String(error));
  ...
  try {
    await runPptLiveBackend(
      'revise_deck',
      `Change the deck to the managed "${selected}" style preset. Re-evaluate both content organization and visual expression under the preset policy. Preserve verified facts and user-authored meaning.`,
      { includeTopic: false },
    );
  } catch (error) {
    state.style = previousStyle;
    syncStylePropertyControls();
    rerender();
    if (isStoppedBackendError(error)) {
      await persist(true);
      return;
    }
    runtime().log?.warn?.('PPT Live managed style transition failed', {
      stylePreset: selected,
      error: String(error),
    });
    failGenerationFromError(error);
...
async function newDeck() {
  ...
  try {
    const manuscriptBase = await captureManuscriptBase();
    const committedManuscript = await resetManuscriptForNewDeck({
      baseDocument: manuscriptBase,
      style: state.style,
    });
    ...
    runtime().log?.warn?.('Failed to reset PPT Live manuscript for a new deck', { error: String(error) });
    setStatus(error instanceof Error ? error.message : String(error));
  } finally {
    localManuscriptCommitDepth -= 1;
    if (localManuscriptCommitDepth === 0) await reconcileObservedManuscript();
...
const handlers = {
  updateOutline(index, value) {
    ...
    if (state.slides[index]) markVisualAhead([state.slides[index].id]);
    rerender();
    void persist(true);
  },
...
function endDrag() {
  const slideId = getActiveSlide(state)?.id;
  dragState = null;
  window.removeEventListener('pointermove', dragMove);
  if (slideId) markVisualAhead([slideId]);
  void persist(true);
}
...
function setWorkspaceMenuOpen(open) {
  ...
  const trigger = $('toggleWorkspaceMenu');
...
function setCommandPanelOpen(open) {
  const panel = $('agentSidebar');
  const trigger = $('openCommandPanel');
  if (!panel || !trigger) return;
  panel.hidden = !open;
  trigger.setAttribute('aria-expanded', String(open));
  document.querySelector('.ppt-live')?.classList.toggle('is-command-panel-open', open);
  if (open) {
    setWorkspaceMenuOpen(false);
    const history = $('historyDrawer');
...
function bindEvents() {
  ...
  $('openCommandPanel')?.addEventListener('click', () => {
    setCommandPanelOpen($('agentSidebar')?.hidden !== false);
  });
  $('closeCommandPanel')?.addEventListener('click', () => setCommandPanelOpen(false));
  $('toggleWorkspaceMenu')?.addEventListener('click', () => {
  ...
  $('toggleHistory')?.addEventListener('click', () => {
    ...
    setWorkspaceMenuOpen(false);
    if (!drawer.hidden) setCommandPanelOpen(false);
  });
  ...
  document.addEventListener('pointerdown', (event) => {
    const wrap = document.querySelector('.workspace-menu-wrap');
    if (wrap && !wrap.contains(event.target)) setWorkspaceMenuOpen(false);
  });
  ...
  $('sendPrompt')?.addEventListener('click', () => {
    if (promptValue()) setCommandPanelOpen(false);
  ...
  $('previewDeck')?.addEventListener('click', () => {
    setWorkspaceMenuOpen(false);
    openPreview();
  });
...
function bindPropertyPanels() {
  ...
  if (stylePresetSelect) {
    ...
    stylePresetSelect.addEventListener('change', () => {
      ...
      if (selected) void changeManagedStylePreset(selected);
    });
  }
}
...
let visualInitialization = null;

async function ensureVisualInitialized() {
  if (visualInitialization) return visualInitialization;
  visualInitialization = (async () => {
  syncLocale();
  try {
    await loadState();
    await recoverFromRestart();
    try {
      observeManuscriptDocument(await captureManuscriptBase());
    } catch (error) {
      runtime().log?.warn?.('Failed to reconcile PPT Live visual state with its manuscript', {
        error: String(error),
      });
    }
    visualStateReady = true;
    await reconcileObservedManuscript({ persistChanges: false });
    syncLocale();
    await persist(true);
  } catch (error) {
...
bindManuscriptController({
  onVisualRoute: ensureVisualInitialized,
  onManuscriptCommitted: handleVisualManuscriptCommitted,
});
