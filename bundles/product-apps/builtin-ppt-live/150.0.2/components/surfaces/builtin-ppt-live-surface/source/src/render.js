...
export function renderAll(state, handlers) {
  ...
  root?.setAttribute('data-density', state.style.density);
  root?.classList.toggle('has-deck', Boolean(state.slides?.length));
...
/** Prefer client box; fall back to border box when WebView/grid reports 0 in the runtime iframe. */
...
export function syncInputs(state) {
  ...
  const exportButton = byId('exportPptx');
  const previewButton = byId('previewDeck');
  if (exportButton) exportButton.disabled = !hasDeck;
  if (previewButton) previewButton.disabled = !hasDeck;
...
export function renderSlideCanvas(state, handlers) {
  ...
  if (!slide) {
    ...
    canvas.innerHTML = isGenerating
      ...
      : `<div class="welcome-hero"><span class="welcome-hero__icon" aria-hidden="true">PL</span><h2>${escapeHtml(t('welcomeTitle'))}</h2><p>${escapeHtml(t('welcomeSubcopy'))}</p></div>`;
