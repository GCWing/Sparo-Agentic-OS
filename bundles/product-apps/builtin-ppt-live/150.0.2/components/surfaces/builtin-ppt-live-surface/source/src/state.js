...
export function createInitialState() {
  const state = {
    ...
    manuscriptBinding: normalizeManuscriptBinding(),
    visualCoverage: normalizeVisualCoverage(),
    slides: [],
    activeSlideId: '',
    selectedElementId: '',
    mode: 'edit',
...
export function ensureState(value) {
  ...
  state.manuscriptBinding = normalizeManuscriptBinding(state.manuscriptBinding);
  state.visualCoverage = normalizeVisualCoverage(state.visualCoverage);
...
export function normalizeManuscriptBinding(value = {}) {
  const status = ['unbound', 'aligned', 'stale', 'visual_ahead', 'conflict'].includes(value?.status)
    ? value.status
    : 'unbound';
  return {
    basedOnRevision: Math.max(0, Number(value?.basedOnRevision) || 0),
    basedOnContentHash: typeof value?.basedOnContentHash === 'string'
      ? value.basedOnContentHash
      : '',
    latestRevision: Math.max(0, Number(value?.latestRevision) || 0),
    latestContentHash: typeof value?.latestContentHash === 'string'
...
export function normalizeVisualCoverage(value = {}) {
  return {
    missingSlideNumbers: Array.isArray(value?.missingSlideNumbers)
      ? [...new Set(value.missingSlideNumbers
        .map(Number)
...
export function normalizeSlide(slide, index, state) {
  ...
  const normalized = {
    ...
    manuscriptBinding: normalizeManuscriptBinding(slide?.manuscriptBinding),
    elements: [],
  };
