...
export interface PermissionReviewEvidenceInput {
  ...
  permissionSummary: AppStudioPermissionSummary;
  observedAt: number;
}
...
export function buildPermissionReviewPreviewResult(
  ...
  const status: WorkStudioFactStatus = failed ? 'failed' : 'passed';
    ...
    checks: [
      {
        ...
        status,
        detail: failed
          ? `App Studio requires app.json package permissions and Runtime Host permission claims to agree before review can pass: ${mismatchDetail}.`
      ...
      },
      {
        ...
        status,
        detail: failed
          ? `App Studio cannot record passing permission review while app.json and Runtime Host permission evidence disagree: ${mismatchDetail}.`
          : `App Studio recorded explicit permission review evidence at ${new Date(input.observedAt).toISOString()}.`,
      },
    ],
    workId: input.workId,
...
function runtimePermissionClaimNames(permissionSummary: AppStudioPermissionSummary): string[] {
  const names = new Set<string>();
  if (permissionSummary.readsWorkspace) names.add('workspace.read');
  if (permissionSummary.writesWorkspace) names.add('workspace.write');
  if (permissionSummary.shellEnabled) names.add('shell');
