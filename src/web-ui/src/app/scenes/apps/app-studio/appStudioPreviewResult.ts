...
export interface AppStudioPreviewProjectionContext {
  workId: WorkId;
  turnId?: string | null;
...
export function buildWorkStudioPreviewResultFromToolResult(
  result: Record<string, unknown>,
  context: AppStudioPreviewProjectionContext,
): WorkStudioPreviewResult {
  ...
  const checks = Array.isArray(result.checks)
    ? result.checks
      ...
      .map((check) => ({
        ...
        status: normalizeStudioFactStatus(String(check.status ?? 'notVerified')),
  ...
  return {
    ...
    kind: normalizeStudioPreviewKind(result.kind),
    status: normalizeStudioFactStatus(String(result.status ?? 'notVerified')),
    source: normalizeStudioPreviewSource(result.source),
    harnessMode,
...
export function normalizeStudioFactStatus(status: string): WorkStudioFactStatus {
  if (
    status === 'passed' ||
    status === 'warning' ||
    status === 'failed' ||
...
export function normalizeStudioPreviewKind(kind: unknown): WorkStudioPreviewResult['kind'] {
  if (
...
export function normalizeStudioPreviewSource(source: unknown): WorkStudioPreviewResult['source'] {
  if (
    source === 'runtime-fact' ||
