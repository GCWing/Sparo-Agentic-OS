...
import type { ProductAppStudioPreviewTarget } from '../product-app-runtime/productAppRuntimeService';

export interface AppStudioPermissionSummary {
...
export interface BuildAppStudioFactsInput {
  productApp: ProductAppCatalogEntry | null;
  componentSubject?: AppStudioComponentSubjectFacts | null;
  hostSurface: ProductAppHostSurface | null;
  previewTarget: ProductAppStudioPreviewTarget | null;
  ...
  validationSummary: AppStudioValidationSummary | null;
  persistedFacts?: AppStudioFacts | null;
  observedAt: number;
  packageRoot?: string | null;
}

export interface AppStudioComponentSubjectFacts {
  componentId: string;
...
function issueId(parts: readonly unknown[]): string {
  return `studio-issue:${parts.map(compactIdPart).join(':')}`;
}

function logId(parts: readonly unknown[]): string {
  return `studio-log:${parts.map(compactIdPart).join(':')}`;
}

function normalizeIssueSeverity(value: unknown): AppStudioIssue['severity'] {
  return value === 'warning' || value === 'noise' ? value : 'fatal';
}

function normalizeLogLevel(value: unknown): AppStudioRuntimeLog['level'] {
  return value === 'debug' || value === 'warn' || value === 'error' ? value : 'info';
}

function issueMergeKey(issue: AppStudioIssue): string {
  return [
...
function logMergeKey(log: AppStudioRuntimeLog): string {
  return [
...
function graphIssueToStudioIssue(issue: WorkRuntimeIssue, hostAppId?: string): AppStudioIssue {
  return {
...
function graphStudioIssueToStudioIssue(issue: WorkStudioIssue): AppStudioIssue {
  return {
    id: issue.id,
    appId: issue.appId,
...
function graphPreviewResultToStudioPreviewResult(preview: WorkStudioPreviewResult): AppStudioFacts['previewResults'][number] {
  const result: AppStudioFacts['previewResults'][number] = {
    id: preview.id,
    kind: preview.kind,
...
function graphValidationResultToValidationSummary(
  validation: WorkStudioValidationResult,
): AppStudioValidationSummary {
  return {
...
function graphLogToStudioLog(log: WorkRuntimeLog, hostAppId?: string): AppStudioRuntimeLog {
  return {
...
export function issuesFromExecutionGraph(
  ...
  if (graph.studioIssues.length > 0) {
    return graph.studioIssues
      .filter((issue) =>
        issue.status !== 'fixed' &&
      ...
      .map(graphStudioIssueToStudioIssue);
  }
  return graph.issues
    .filter((issue) =>
    ...
    .map((issue) => graphIssueToStudioIssue(issue, runtimeContext?.hostSurfaceId));
}
...
export function previewResultsFromExecutionGraph(
  ...
  const runtimeInstanceId = runtimeContext?.runtimeInstanceId;
  const productAppId = runtimeContext?.productAppId;
  return graph.studioPreviewResults
    .filter((preview) =>
    ...
    .map(graphPreviewResultToStudioPreviewResult);
}

export function validationSummaryFromExecutionGraph(
  graph: WorkExecutionGraph | null,
  productApp: ProductAppCatalogEntry | null,
  componentSubject: AppStudioComponentSubjectFacts | null | undefined,
): AppStudioValidationSummary | null {
  if (!graph) return null;
  const candidates = graph.studioValidationResults
    .filter((validation) => {
      if (productApp) {
...
export function logsFromExecutionGraph(
  ...
  return graph.logs
    ...
    .map((log) => graphLogToStudioLog(log, runtimeContext?.hostSurfaceId));
}

export function mergeStudioIssues(...groups: AppStudioIssue[][]): AppStudioIssue[] {
  const byKey = new Map<string, AppStudioIssue>();
...
export function mergeStudioLogs(...groups: AppStudioRuntimeLog[][]): AppStudioRuntimeLog[] {
  const byKey = new Map<string, AppStudioRuntimeLog>();
...
export function normalizeProductAppValidationSummary(
  ...
  const checks = Array.isArray(data.checks)
    ? data.checks
        ...
        .map((check) => ({
          ...
          status: String(check.status ?? 'notVerified') as AppStudioValidationSummary['status'],
  ...
  const status = String(data.status ?? deriveValidationStatusFromChecks(checks, failed, warnings)) as AppStudioValidationSummary['status'];
  return {
    status,
    failed,
    warnings,
...
function deriveValidationStatusFromChecks(
  checks: AppStudioValidationSummary['checks'],
  failed: number,
  warnings: number,
): AppStudioValidationSummary['status'] {
...
function buildDerivedComponentValidationSummary(
  componentSubject: AppStudioComponentSubjectFacts,
  issues: AppStudioIssue[],
  updatedAt: number,
): AppStudioValidationSummary {
  const fatalCount = issues.filter((issue) => issue.severity === 'fatal').length;
...
function deriveReleaseStatus(
  ...
  issues: AppStudioIssue[],
  previewResults: AppStudioFacts['previewResults'],
): AppStudioFactStatus {
...
function latestPreviewCheck(
  previewResults: AppStudioFacts['previewResults'],
  checkId: string,
  kind?: AppStudioFacts['previewResults'][number]['kind'],
): { status: AppStudioFactStatus | (string & {}); detail?: string } | null {
...
export function buildAppStudioFacts(input: BuildAppStudioFactsInput): AppStudioFacts {
  const {
    productApp,
    componentSubject,
    hostSurface,
  ...
  return {
    ...
    blueprint: productApp
      ? {
          whatItDoes: productApp.description || productApp.goal,
