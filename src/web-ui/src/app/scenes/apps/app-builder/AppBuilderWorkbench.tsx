...
import {
  ...
  resolveProductAppBuilderPreviewTarget,
  type ProductAppBuilderPreviewFailureContext,
  type ProductAppBuilderPreviewTarget,
...
import {
  ...
  type AppBuilderPermissionSummary,
  type AppBuilderComponentSubjectFacts,
} from './appBuilderFacts';
import {
  buildWorkBuilderPreviewResultFromToolResult,
  compactFactId,
  normalizeBuilderFactStatus,
  stringFromRecord,
} from './appBuilderPreviewResult';
import {
  buildRuntimeBoundaryPreviewResult,
  type RuntimeBoundaryDataProbe,
  type RuntimeBoundaryStorageProbe,
} from './appBuilderRuntimeBoundaryEvidence';
import { buildRuntimeDependencyPreviewResult } from './appBuilderRuntimeDependencyEvidence';
import {
...
type AppBuilderLatestRelease = NonNullable<NonNullable<AppBuilderFacts['versionSummary']>['latestRelease']>;
type WorkbenchTabId =
  | 'blueprint'
  | 'preview'
  | 'issues'
...
type PreviewObservationOptions = {
  status?: WorkBuilderPreviewResult['status'];
  source?: WorkBuilderPreviewResult['source'];
  detail?: string;
  runtimeReady?: ProductAppRuntimeReadyPayload | null;
  interactionProbe?: ProductAppRuntimeInteractionProbePayload | null;
  checks?: WorkBuilderPreviewResult['checks'];
  issueCount?: number;
  fatalIssueCount?: number;
  warningIssueCount?: number;
  triggerTurnId?: string | null;
};
type WorkBuilderPreviewCheck = NonNullable<WorkBuilderPreviewResult['checks']>[number];
...
interface IssueRowProps {
  issue: AppBuilderIssue;
  t: (key: string, opts?: Record<string, unknown>) => string;
  onCopy: (text: string) => void;
  onRecompile: () => void;
  onRestart: () => void;
  onFixWithAi: (issue: AppBuilderIssue) => void;
...
const IssueRow: React.FC<IssueRowProps> = ({
  ...
  return (
    <div className={`builder-issue is-${issue.severity}`}>
      <Button
        variant="ghost"
        size="small"
        className="builder-issue__summary"
        onClick={() => setExpanded((v) => !v)}
        aria-expanded={expanded}
      >
        <span className="builder-issue__severity-dot" />
        <span className="builder-issue__message">{issue.message}</span>
        <span className="builder-issue__meta">
          {issue.category ? <span className="builder-issue__category">{issue.category}</span> : null}
          <span>{formatRuntimeTimestamp(issue.timestampMs, currentLanguage)}</span>
        </span>
        <span className="builder-issue__chevron">
          {expanded ? <ChevronUp size={11} /> : <ChevronDown size={11} />}
        </span>
      </Button>

      {expanded ? (
        <div className="builder-issue__detail">
          {hintKey ? (
            <div className="builder-issue__hint">
              {t(`diagnostics.hints.${hintKey}`)}
            </div>
          ) : null}
          {detailText ? <pre className="builder-issue__pre">{detailText}</pre> : null}
          <div className="builder-issue__actions">
...
const LogRow: React.FC<LogRowProps> = ({ entry, onCopy, currentLanguage, copyAriaLabel }) => {
  ...
  return (
    <div className={`builder-log is-${entry.level}`}>
      <Button
        variant="ghost"
        size="small"
        className="builder-log__summary"
        onClick={() => hasDetail && setExpanded((v) => !v)}
        aria-expanded={hasDetail ? expanded : undefined}
        style={{ cursor: hasDetail ? 'pointer' : 'default' }}
      >
        <span className="builder-log__level-bar" />
        <span className="builder-log__message">{entry.message}</span>
        <span className="builder-log__meta">
          <span className="builder-log__category">{entry.level}/{entry.category}</span>
          <span>{formatRuntimeTimestamp(entry.timestampMs, currentLanguage)}</span>
        </span>
        {hasDetail ? (
          <span className="builder-log__chevron">
            {expanded ? <ChevronUp size={11} /> : <ChevronDown size={11} />}
          </span>
        ) : null}
      </Button>
      ...
      {expanded && detailText ? (
        <div className="builder-log__detail">
          <pre className="builder-log__pre">{detailText}</pre>
          <div className="builder-log__actions">
            <IconButton
              type="button"
              variant="ghost"
              size="xs"
  ...
  <section className="builder-fact-section">
    ...
    {rows.length > 0 ? (
      <div className="builder-fact-grid">
        {rows.map((row) => (
          <div className="builder-fact-row" key={row.label}>
            <span className="builder-fact-row__label">{row.label}</span>
            <span className={`builder-fact-row__value${row.state ? ` is-${row.state}` : ''}`}>
              {row.value}
            </span>
          </div>
        ))}
      ...
      <div className="builder-fact-section__empty">{empty}</div>
    )}
  </section>
);
...
function previewCheck(
  id: string,
  status: WorkBuilderPreviewCheck['status'],
  detail: string,
): WorkBuilderPreviewCheck {
...
function countPreviewChecks(
  checks: readonly WorkBuilderPreviewCheck[],
  statuses: readonly WorkBuilderPreviewCheck['status'][],
): number {
...
function derivePreviewObservationStatus(
  checks: readonly WorkBuilderPreviewCheck[],
): WorkBuilderPreviewResult['status'] {
...
function previewSurfaceDescriptorForTarget(target: ProductAppBuilderPreviewTarget): PreviewSurfaceDescriptor {
...
function previewSurfaceDescriptorForFailureContext(
  context: ProductAppBuilderPreviewFailureContext,
): PreviewSurfaceDescriptor {
...
function buildPreviewObservationChecks(
  target: ProductAppBuilderPreviewTarget,
  ...
  const interactionSurfaceStatus: WorkBuilderPreviewCheck['status'] =
...
function buildConsumerCompatibilityPreviewResults(
  ...
  observedAt: number,
  runtimeReady: ProductAppRuntimeReadyPayload | null,
  primaryChecks: readonly WorkBuilderPreviewCheck[],
  primaryStatus: WorkBuilderPreviewResult['status'],
): WorkBuilderPreviewResult[] {
  ...
  return components.map((component) => {
    ...
    const status: WorkBuilderPreviewResult['status'] = canPass ? 'passed' : 'notVerified';
...
): WorkBuilderPreviewCheck['status'] {
...
function persistedAppBuilderFactsForSession(sessionId: string | null): AppBuilderFacts | null {
  if (!sessionId) return null;
  return flowChatStore.getState().sessions.get(sessionId)?.customMetadata?.appBuilderFacts ?? null;
}

function runtimeHostPermissionSummary(app: ProductAppHostSurface): AppBuilderPermissionSummary {
...
async function recordPreviewObservationForTarget(
  ...
  const previewResult: WorkBuilderPreviewResult = {
  ...
  await agenticOsWorkApi.recordBuilderPreviewResult({
    workId: target.work.id,
    previewResult,
  });
...
async function recordPreviewObservationForFailureContext(
  ...
  const checks: WorkBuilderPreviewCheck[] = [
    previewCheck(
      'surfaceMode',
      previewSurface.verified ? 'passed' : 'notVerified',
      previewSurface.detail,
...
const AppBuilderWorkbench: React.FC<AppBuilderWorkbenchProps> = ({
  sessionId,
  appId,
  componentId,
  componentKind,
  ...
  const { currentLanguage, t } = useI18n('scenes/app-builder');
  ...
  const [previewTarget, setPreviewTarget] = useState<ProductAppBuilderPreviewTarget | null>(null);
  ...
  const [runtimeEventIssues, setRuntimeEventIssues] = useState<AppBuilderIssue[]>([]);
  const [runtimeEventLogs, setRuntimeEventLogs] = useState<AppBuilderRuntimeLog[]>([]);
  const [validationSummary, setValidationSummary] = useState<AppBuilderValidationSummary | null>(null);
  ...
  const [restoreReleaseTarget, setRestoreReleaseTarget] = useState<AppBuilderLatestRelease | null>(null);
  ...
  const previewTargetRef = useRef<ProductAppBuilderPreviewTarget | null>(null);
  ...
  const subscribeAppBuilderFacts = useCallback((notify: () => void) => flowChatStore.subscribeSelector(
      (state) => (sessionId
        ? state.sessions.get(sessionId)?.customMetadata?.appBuilderFacts ?? null
        : null),
      () => notify(),
    ), [sessionId]);
  const persistedAppBuilderFacts = useSyncExternalStore(
    subscribeAppBuilderFacts,
    () => persistedAppBuilderFactsForSession(sessionId),
    () => persistedAppBuilderFactsForSession(sessionId),
  );
  ...
  const componentSubject = useMemo<AppBuilderComponentSubjectFacts | null>(() => {
  ...
  const builderWorkId = previewTarget?.work.id ?? componentWork?.id;
  ...
  const permissionSummary = useMemo<AppBuilderPermissionSummary | null>(() => {
    if (!app) return null;
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const target = await resolveProductAppBuilderPreviewTarget(appId, {
      ...
      const message = err instanceof Error ? err.message : String(err);
      setError(message);
      if (isProductAppBuilderPreviewResolveError(err)) {
        try {
  ...
  useEffect(() => {
    ...
    const unlistenIssue = api.listen<Record<string, unknown>>(productAppRuntimeHostEvents.runtimeIssue, (payload) => {
      ...
      if (!issue) return;
      setRuntimeEventIssues((current) => mergeBuilderIssues([issue], current).slice(0, MAX_VISIBLE_ISSUES));
    ...
    const unlistenLog = api.listen<Record<string, unknown>>(productAppRuntimeHostEvents.runtimeLog, (payload) => {
      ...
      if (!logEntry) return;
      setRuntimeEventLogs((current) => mergeBuilderLogs(current, [logEntry]).slice(-MAX_VISIBLE_LOGS));
  ...
  useEffect(() => {
    const handleValidationResult = (event: Event) => {
      ...
      const validationResult: WorkBuilderValidationResult = {
        ...
        checks: summary.checks.map((check) => ({
          id: check.id,
          status: normalizeBuilderFactStatus(String(check.status)),
          detail: check.detail ?? null,
        })),
      };
      try {
        await agenticOsWorkApi.recordBuilderValidationResult({
          workId,
          validationResult,
        });
    ...
    window.addEventListener('app-builder-validation-result', handleValidationResult);
    return () => window.removeEventListener('app-builder-validation-result', handleValidationResult);
  }, [
    componentSubject,
    productApp,
  ...
  useEffect(() => {
    const handlePreviewResult = (event: Event) => {
      void (async () => {
        ...
        const previewResult = buildWorkBuilderPreviewResultFromToolResult(result, {
          workId,
    ...
    window.addEventListener('app-builder-preview-result', handlePreviewResult);
    return () => window.removeEventListener('app-builder-preview-result', handlePreviewResult);
  }, [
    componentSubject,
    productApp,
    refreshExecutionGraph,
  ...
  const issues = useMemo(
    () => mergeBuilderIssues(graphIssues, runtimeEventIssues).slice(0, MAX_VISIBLE_ISSUES),
    [graphIssues, runtimeEventIssues],
  );
  const logs = useMemo(
    () => mergeBuilderLogs(graphLogs, runtimeEventLogs).slice(-MAX_VISIBLE_LOGS),
  ...
  const buildIssuePrompt = useCallback(
    (singleIssue?: AppBuilderIssue) => {
      ...
      const subjectLabel = productApp
        ...
        : componentSubject
          ...
          : appId ?? 'current App Builder subject';
      const subjectKind = productApp ? 'Product App' : componentSubject ? 'Component package' : 'App Builder subject';
      if (singleIssue) {
        const structuredContext = {
          workId: builderWorkId,
        ...
        return [
          ...
          '',
          'Structured Builder issue context:',
      ...
      return [
        `Fix the current ${subjectKind} based on its Builder diagnostics. Subject: ${subjectLabel}`,
    ...
    [appId, componentSubject, filteredLogs, issues, productApp, productAppFacts?.packagePath, runtimeContext?.runtimeInstanceId, builderWorkId],
  );

  const handleSendIssuesToAi = useCallback(
    async (singleIssue?: AppBuilderIssue) => {
            ...
            ? {
                metadata: {
                  appBuilderIssueContext: {
                    ...
                    issueId: singleIssue.id,
                    productAppId: singleIssue.productAppId ?? productApp?.id ?? appId,
                    subjectKind: productApp ? 'Product App' : componentSubject ? 'Component package' : 'App Builder subject',
                    componentKind: componentSubject?.componentKind,
                    runtimeInstanceId: singleIssue.runtimeInstanceId ?? runtimeContext?.runtimeInstanceId,
                    componentId: singleIssue.componentId ?? componentSubject?.componentId,
                    previewResultId: singleIssue.previewResultId,
    ...
    [appId, buildIssuePrompt, componentSubject, filteredLogs.length, issues.length, productApp, productAppFacts?.packagePath, runtimeContext?.runtimeInstanceId, sendingIssues, sessionId, builderWorkId, t],
  );

  // 鈹€鈹€ Permissions submenu entries 鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€
  const buildRestoreReleasePrompt = useCallback((release: AppBuilderLatestRelease): string => {
    ...
    return [
      ...
      'After restore, report restored_files, removed_files, package_digest, and component_lock_digest, then refresh App Builder facts.',
    ].filter(Boolean).join('\n');
  ...
  const releaseSummaryText = useCallback((release: AppBuilderLatestRelease): string => {
    return [
      productApp ? `Product App: ${productApp.id}@${productApp.version}` : '',
  ...
  const handleCopyRelease = useCallback(async (release: AppBuilderLatestRelease) => {
    try {
  ...
  const buildCompareReleasePrompt = useCallback((release: AppBuilderLatestRelease): string => {
  ...
  const handleCompareRelease = useCallback(async (release: AppBuilderLatestRelease) => {
    ...
    try {
      await flowChatManager.sendMessage(
        ...
        undefined,
        {
          ...
          metadata: {
            appBuilderReleaseCompareContext: {
  ...
  const buildTemplateReleasePrompt = useCallback((release: AppBuilderLatestRelease): string => {
    const packageRoot = releasePackageRoot();
    ...
    return [
      ...
      'After creating it, bind the new Product App to this App Builder session and report the new app id, package root, component lock, and validation status.',
    ].filter(Boolean).join('\n');
  }, [appId, productApp, releasePackageRoot]);

  const handleUseReleaseAsTemplate = useCallback(async (release: AppBuilderLatestRelease) => {
    ...
    try {
      await flowChatManager.sendMessage(
        ...
        undefined,
        {
          triggerSource: 'desktop_ui',
          metadata: {
            appBuilderReleaseTemplateContext: {
              releaseId: release.releaseId,
              productAppId: productApp?.id ?? appId,
              productAppVersion: productApp?.version,
              packageRoot: releasePackageRoot(),
    ...
    try {
      await flowChatManager.sendMessage(
        ...
        undefined,
        {
          triggerSource: 'desktop_ui',
          metadata: {
            appBuilderReleaseRestoreContext: {
  ...
  const appBuilderFacts = useMemo<AppBuilderFacts>(() => {
    ...
    return buildAppBuilderFacts({
      ...
      logs,
      validationSummary,
      persistedFacts: persistedAppBuilderFacts,
      observedAt,
  ...
  useEffect(() => {
    if (!sessionId) return;
    const serializedFacts = JSON.stringify(appBuilderFacts);
    let changed = false;
    flowChatStore.setState(prev => {
      const session = prev.sessions.get(sessionId);
      if (!session) return prev;
      if (JSON.stringify(session.customMetadata?.appBuilderFacts ?? null) === serializedFacts) {
        return prev;
      }
      changed = true;
  ...
  const readinessRows = useMemo<FactRow[]>(() => {
    const checks = appBuilderFacts.validationSummary?.checks ?? [];
  ...
  }, [appBuilderFacts.validationSummary, labelForCheck]);

  const evalRows = useMemo<FactRow[]>(() => {
    const summary = appBuilderFacts.evalSummary;
    if (!summary) return [];
    return [
      {
  ...
  const releaseArtifactRows = useMemo<FactRow[]>(() => {
    const latestRelease = appBuilderFacts.versionSummary?.latestRelease;
    const latestPublished = appBuilderFacts.versionSummary?.latestPublishedRelease;
    const rows: FactRow[] = [];
    if (latestRelease) {
      rows.push({
    ...
    return rows;
  }, [appBuilderFacts.versionSummary, currentLanguage, t]);
      ...
      return (
        <div className="builder-workbench-facts">
          <FactSection
            ...
            empty={fallback}
            rows={productApp ? [
              { label: t('workbench.facts.whatItDoes'), value: appBuilderFacts.blueprint?.whatItDoes || displayDescription || t('workbench.values.notSpecified') },
              { label: t('workbench.facts.howIUseIt'), value: appBuilderFacts.blueprint?.howIUseIt || (productApp.launch?.kind === 'agentSession' ? t('workbench.values.agentPreview') : productApp.primarySurfaceMode === 'sidecarLinked' ? t('workbench.values.sidecarPreview') : t('workbench.values.fullPreview')) },
              { label: t('workbench.facts.whatAiDoes'), value: appBuilderFacts.blueprint?.whatAiDoes || (productAppAgentComponents.length > 0 ? joinList(productAppAgentComponents.map(component => component.componentId), '') : t('workbench.values.noBackendActions')), state: productAppAgentComponents.length > 0 ? 'ok' : 'neutral' },
              { label: t('workbench.facts.whatData'), value: appBuilderFacts.blueprint?.whatData || t('workbench.values.packageAndRuntimeState') },
              { label: t('workbench.facts.howReady'), value: appBuilderFacts.blueprint?.howReady || (issueCounts.fatal > 0 ? t('workbench.values.runtimeIssues') : t('workbench.values.previewLoaded')), state: statusState(String(appBuilderFacts.validationSummary?.status ?? 'notVerified')) },
            ] : componentSubject ? [
              { label: t('workbench.facts.whatItDoes'), value: appBuilderFacts.blueprint?.whatItDoes || componentSubject.description || t('workbench.values.notSpecified') },
              { label: t('workbench.facts.howIUseIt'), value: appBuilderFacts.blueprint?.howIUseIt || t('workbench.values.componentConsumerUse') },
              { label: t('workbench.facts.whatAiDoes'), value: appBuilderFacts.blueprint?.whatAiDoes || t('workbench.values.componentCapability') },
              { label: t('workbench.facts.whatData'), value: appBuilderFacts.blueprint?.whatData || t('workbench.values.componentPackageState') },
              { label: t('workbench.facts.howReady'), value: appBuilderFacts.blueprint?.howReady || t('workbench.values.componentReadiness'), state: statusState(String(appBuilderFacts.validationSummary?.status ?? 'notVerified')) },
            ] : []}
          />
          <FactSection
            ...
            empty={fallback}
            rows={productApp ? [
              { label: t('workbench.facts.appId'), value: String(appBuilderFacts.technicalBlueprint?.appId ?? productApp.id) },
              { label: t('workbench.facts.version'), value: String(appBuilderFacts.technicalBlueprint?.version ?? productApp.version) },
              ...
              { label: t('workbench.facts.launchKind'), value: String(appBuilderFacts.technicalBlueprint?.launchKind ?? productApp.launch?.kind ?? t('workbench.values.notSpecified')) },
              { label: t('workbench.facts.runtimeRevision'), value: textOrFallback(appBuilderFacts.technicalBlueprint?.runtimeInstanceId, t('workbench.values.unknown')) },
            ] : componentSubject ? [
              { label: t('workbench.facts.componentId'), value: String(appBuilderFacts.technicalBlueprint?.componentId ?? componentSubject.componentId) },
              { label: t('workbench.facts.componentKind'), value: String(appBuilderFacts.technicalBlueprint?.componentKind ?? componentSubject.componentKind) },
              { label: t('workbench.facts.version'), value: String(appBuilderFacts.technicalBlueprint?.version ?? componentSubject.version ?? t('workbench.values.unknown')) },
              { label: t('workbench.facts.packageRoot'), value: String(appBuilderFacts.technicalBlueprint?.packageRoot ?? componentSubject.packageRoot ?? t('workbench.values.unknown')) },
            ] : []}
          />
        </div>
      );
    ...
    if (activeTab === 'issues') {
      return (
        <div className="builder-workbench-facts">
          ...
          />
          <div className="builder-workbench-issue-list">
            {issues.length > 0 ? issues.map((issue, index) => (
              ...
              <div className="builder-fact-section__empty">{t('diagnostics.empty')}</div>
            )}
          </div>
        </div>
      );
      ...
      return (
        <div className="builder-workbench-facts">
          <FactSection
            ...
            rows={hasSubject ? [
              { label: t('workbench.facts.backendActions'), value: appBuilderFacts.agentSummary?.backendActionCount ?? productAppAgentComponents.length, state: (appBuilderFacts.agentSummary?.backendActionCount ?? 0) > 0 ? 'ok' : 'neutral' },
              { label: t('workbench.facts.memoryScope'), value: joinList(appBuilderFacts.agentSummary?.memoryScopes, t('workbench.values.none')) },
              { label: t('workbench.facts.sessionPolicy'), value: joinList(appBuilderFacts.agentSummary?.sessionPolicies, t('workbench.values.none')) },
            ] : []}
          />
        </div>
      );
      ...
      return (
        <div className="builder-workbench-facts">
          <FactSection
            ...
            empty={fallback}
            rows={hasSubject && appBuilderFacts.dataSummary ? [
              { label: t('workbench.facts.inputData'), value: appBuilderFacts.dataSummary.readsWorkspace ? t('workbench.values.workspaceReadable') : t('workbench.values.userProvidedOnly') },
              { label: t('workbench.facts.workingState'), value: `${t('workbench.values.runtimeScoped')} - ${appBuilderFacts.dataSummary.runtimeRunCount}` },
              { label: t('workbench.facts.externalFacts'), value: appBuilderFacts.dataSummary.externalAccess ? t('workbench.values.externalAccess') : t('workbench.values.noExternalAccess'), state: appBuilderFacts.dataSummary.externalAccess ? 'warning' : 'ok' },
              { label: t('workbench.facts.retention'), value: `${t(lifecyclePolicyLabelKey(appBuilderFacts.dataSummary.retentionPolicy))} - ${appBuilderFacts.dataSummary.artifactCount}` },
              { label: t('workbench.facts.deletion'), value: t(lifecyclePolicyLabelKey(appBuilderFacts.dataSummary.deletionPolicy)) },
              { label: t('workbench.facts.migration'), value: t(lifecyclePolicyLabelKey(appBuilderFacts.dataSummary.migrationPolicy)) },
              { label: t('workbench.facts.sharePolicy'), value: t(lifecyclePolicyLabelKey(appBuilderFacts.dataSummary.sharePolicy)) },
            ] : []}
          />
        </div>
      );
      ...
      return (
        <div className="builder-workbench-facts">
          <FactSection
            ...
            empty={fallback}
            rows={hasSubject ? [
              { label: t('workbench.facts.currentDraft'), value: appBuilderFacts.versionSummary?.currentVersion ?? productApp?.version ?? componentSubject?.version ?? t('workbench.values.unknown') },
              { label: t('workbench.facts.sourceRevision'), value: textOrFallback(appBuilderFacts.versionSummary?.sourceRevision, t('workbench.values.unknown')) },
              { label: t('workbench.facts.lockDigest'), value: appBuilderFacts.versionSummary?.componentLockDigest ?? productApp?.componentLockDigest ?? productApp?.componentLockId ?? t('workbench.values.notSpecified'), state: 'ok' },
              { label: t('workbench.facts.releaseStatus'), value: appBuilderFacts.versionSummary?.releaseStatus ?? releaseRehearsal?.status ?? t('workbench.values.notVerified'), state: statusState(String(appBuilderFacts.versionSummary?.releaseStatus ?? releaseRehearsal?.status ?? 'notVerified')) },
              ...releaseArtifactRows,
          ...
          {productApp && latestRelease ? (
            <div className="builder-workbench-facts__actions">
              ...
              >
                {requestingReleaseCompare ? <DotMatrixLoader size="tiny" className="builder-spin" /> : <GitCompareArrows size={12} />}
              ...
              >
                {requestingReleaseTemplate ? <DotMatrixLoader size="tiny" className="builder-spin" /> : <Send size={12} />}
              ...
              >
                {requestingReleaseRestore ? <DotMatrixLoader size="tiny" className="builder-spin" /> : null}
      ...
      return (
        <div className="builder-workbench-facts">
          ...
          <div className="builder-workbench-facts__actions">
            ...
            >
              {recordingPermissionReview ? <DotMatrixLoader size="tiny" className="builder-spin" /> : null}
    ...
    return (
      <div className="builder-workbench-facts">
        <FactSection
          ...
          empty={fallback}
          rows={hasSubject && appBuilderFacts.shareSummary ? [
            {
              label: t('workbench.facts.visibility'),
              value: visibilityLabel(appBuilderFacts.shareSummary.visibility, t),
            },
            { label: t('workbench.facts.installLocation'), value: appBuilderFacts.shareSummary.installLocation === 'workspace' ? t('workbench.values.workspaceInstall') : t('workbench.values.systemInstall') },
            { label: t('workbench.facts.privateData'), value: appBuilderFacts.shareSummary.privateDataExcluded ? t('workbench.values.privateDataExcluded') : t('workbench.values.notVerified'), state: appBuilderFacts.shareSummary.privateDataExcluded ? 'ok' : 'warning' },
          ] : []}
        />
      </div>
    );
  ...
  return (
    <>
      ...
      />
      <div className={`app-builder-panel${dockState === 'collapsed' ? ' is-dock-collapsed' : ''}`}>
      ...
      <div className="builder-statusbar">
        <div className="builder-statusbar__identity">
          <span className={`builder-statusbar__dot ${runtimeDotClass}`} />
          <span className="builder-statusbar__name">{displayName}</span>
          {runtimeSummary?.runtimeLabel ? (
            <span className="builder-statusbar__runtime-label">{runtimeSummary.runtimeLabel}</span>
          ) : null}
        </div>
        ...
        <div className="builder-statusbar__ctas">
          {runtimeSummary?.depsDirty ? (
            <Button
              variant="secondary"
              size="small"
              className="builder-statusbar__cta is-warning"
              onClick={() => void actions.installDeps(() => void load())}
              disabled={actions.state.installingDeps}
            >
              {actions.state.installingDeps ? <DotMatrixLoader size="tiny" className="builder-spin" /> : null}
          ...
          {runtimeSummary?.workerRestartRequired && !isRunning ? (
            ...
            >
              {actions.state.restartingWorker ? <DotMatrixLoader size="tiny" className="builder-spin" /> : null}
          ...
          {isRunning ? (
            <Button
              variant="secondary"
              size="small"
              className="builder-statusbar__cta is-running"
        ...
        <span className="builder-statusbar__sep" aria-hidden="true" />
        ...
        <div className="builder-statusbar__actions">
          ...
          >
            {loading ? <DotMatrixLoader size="tiny" className="builder-spin" /> : <RefreshCw size={13} />}
          </IconButton>
          <IconButton
            variant="ghost"
            size="xs"
      ...
      <div className="builder-workbench-tabs">
        ...
        />
        <div className="builder-workbench-tabs__status">
          <Badge
      ...
      {activeTab === 'preview' ? (
        ...
        {appId && loading && !app ? (
          <div className="builder-preview__empty">
            <DotMatrixLoader size="medium" className="builder-spin" />
        ...
        {loading && app ? (
          <div className="builder-preview__updating" role="status" aria-live="polite">
            <DotMatrixLoader size="tiny" className="builder-spin" />
            <span>{t('panel.updating')}</span>
          </div>
        ) : null}
        {app && previewInspectorEnabled ? (
          <div className="builder-preview-inspector-status" role="status" aria-live="polite">
        ...
        {previewSelection ? (
          <div className="builder-preview-selection-tray">
            <span className="builder-preview-selection-tray__label">
              {t('previewSelection.contextLabel')}
            </span>
            <span className="builder-preview-selection-tray__summary" title={previewSelectionSummary || undefined}>
            ...
            >
              {addingPreviewSelection ? <DotMatrixLoader size="tiny" className="builder-spin" /> : null}
      ...
      <div className="builder-dock">
        ...
        <div className="builder-dock__header">
          <Button
            variant="ghost"
            size="small"
            className="builder-dock__toggle"
            onClick={() => setDockState((s) => (s === 'collapsed' ? 'open' : 'collapsed'))}
            aria-expanded={dockState === 'open'}
          >
            <span className="builder-dock__title">{t('diagnostics.title')}</span>
            <span className={`builder-dock__status ${dockStatusClass}`}>{dockStatusLabel}</span>
            <span className="builder-dock__chevron">
              {dockState === 'open' ? <ChevronDown size={11} /> : <ChevronUp size={11} />}
            </span>
          </Button>

          <div className="builder-dock__header-actions">
            <IconButton
              variant="ghost"
              size="xs"
            ...
            >
              {sendingIssues ? <DotMatrixLoader size="tiny" className="builder-spin" /> : <Send size={12} />}
            </IconButton>
            <IconButton
              variant="ghost"
              size="xs"
            ...
            >
              {clearingIssues ? <DotMatrixLoader size="tiny" className="builder-spin" /> : <Trash2 size={12} />}
            </IconButton>
          </div>
        </div>
        ...
        {/* Body - only when open */}
        {dockState === 'open' ? (
          <div className="builder-dock__body">
            {/* Tab bar: Issues | Logs */}
            <div className="builder-dock__tabs">
              <Button
                variant="ghost"
                size="small"
                className={`builder-dock__tab${runtimeView === 'issues' ? ' is-active' : ''}`}
                onClick={() => setRuntimeView('issues')}
              >
                {t('diagnostics.issuesTab')}
                {issueCounts.total > 0 ? (
                  <span className={`builder-dock__tab-badge ${issueCounts.fatal > 0 ? 'is-fatal' : 'is-warning'}`}>
              ...
              <Button
                variant="ghost"
                size="small"
                className={`builder-dock__tab${runtimeView === 'logs' ? ' is-active' : ''}`}
                onClick={() => setRuntimeView('logs')}
              >
                {t('diagnostics.logsTab')}
                {filteredLogs.length > 0 ? (
                  <span className="builder-dock__tab-badge is-neutral">
            ...
            {runtimeView === 'logs' ? (
              <div className="builder-dock__log-controls">
                <FilterPillGroup className="builder-dock__log-filter-group">
                ...
                <Search
                  className="builder-dock__log-search-field"
                  value={logSearch}
            ...
            {/* List */}
            <div
              className="builder-dock__list"
              ref={logsScrollRef}
              onScroll={runtimeView === 'logs' ? handleLogsScroll : undefined}
            >
              {runtimeView === 'issues' ? (
                issues.length === 0 ? (
                  <div className="builder-dock__empty">{t('diagnostics.empty')}</div>
                ) : (
                ...
                <EmptyState
                  className="builder-dock__logs-empty"
                ...
                <>
                  {logs.length >= MAX_VISIBLE_LOGS ? (
                    <Alert
                      type="info"
                      className="builder-dock__truncated-alert"
            ...
            {runtimeView === 'logs' && !followTail && newLogCount > 0 ? (
              <Button
                ...
                variant="accent"
                size="small"
                className="builder-dock__new-logs-banner"
