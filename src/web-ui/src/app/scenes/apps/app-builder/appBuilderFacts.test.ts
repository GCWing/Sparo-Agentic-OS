...
describe('appBuilderFacts', () => {
  ...
  it('deduplicates graph and live event issues while preserving live origin', () => {
    ...
    const merged = mergeBuilderIssues(
    ...
    expect(merged[0].id).toContain('builder-issue:');
  });
  ...
  it('prefers core Builder issues and preview results from Work graph', () => {
    const graphWithBuilderFacts: WorkExecutionGraph = {
      ...graph,
      builderPreviewResults: [{
    ...
    const facts = buildAppBuilderFacts({
      productApp,
      hostSurface: null,
      previewTarget: null,
      runtimeContext,
      executionGraph: graphWithBuilderFacts,
      sourceFileCount: 4,
      permissionSummary: null,
      runtimeHasAttention: false,
      issues: issuesFromExecutionGraph(graphWithBuilderFacts, runtimeContext),
      logs: logsFromExecutionGraph(graphWithBuilderFacts, runtimeContext),
  ...
  it('keeps blocked preview facts without runtime identity', () => {
    const graphWithBlockedPreview: WorkExecutionGraph = {
      ...
      builderValidationResults: [],
      builderIssues: [{
        id: 'builder-issue:preview:work-1:runtime-resolve',
        appId: 'app-1',
        productAppId: 'app-1',
        componentId: 'surface-1',
        runtimeInstanceId: null,
  ...
  it('prefers core Builder validation results from Work graph', () => {
