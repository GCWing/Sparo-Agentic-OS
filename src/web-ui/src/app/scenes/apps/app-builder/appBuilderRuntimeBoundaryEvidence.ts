...
function buildRuntimeBoundaryChecks(
  input: RuntimeBoundaryEvidenceInput,
): NonNullable<WorkBuilderPreviewResult['checks']> {
  ...
  const dataStatus: WorkBuilderFactStatus = !storageProbePassed || dataProbeFailed
    ? 'failed'
    : dataProbePassed && dataBoundaryDeclared
      ? 'passed'
      : 'notVerified';
  const dataLifecycleStatus: WorkBuilderFactStatus = !storageProbePassed || dataProbeFailed
    ? 'failed'
    : dataProbePassed && dataLifecycleDeclared
      ? 'passed'
      : 'notVerified';
  const dataSummaryStatus: WorkBuilderFactStatus = !storageProbePassed || dataProbeFailed
    ? 'failed'
...
function derivePreviewStatus(
  checks: NonNullable<WorkBuilderPreviewResult['checks']>,
): WorkBuilderFactStatus {
...
function detailForStatus(status: WorkBuilderFactStatus): string {
