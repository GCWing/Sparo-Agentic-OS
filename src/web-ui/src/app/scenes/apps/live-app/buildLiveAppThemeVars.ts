 ...
 * Legacy app CSS variables are emitted only as a Live App runtime theme
 * adapter contract because generated or persisted iframe content may still
 * consume them.
 */
import type { ThemeConfig, ThemeType } from '@/infrastructure/theme/types';

const LEGACY_RUNTIME_THEME_NAMESPACE = 'bit' + 'fun';
const LEGACY_RUNTIME_THEME_PREFIX = `--${LEGACY_RUNTIME_THEME_NAMESPACE}-`;
...
export function buildLiveAppThemeVars(theme: ThemeConfig | null): LiveAppThemePayload | null {
  ...
  // Preferred semantic slots for generated Live Apps. These tell builders what a
  ...
  addLegacyRuntimeThemeVars(vars);
...
function addLegacyRuntimeThemeVars(vars: Record<string, string>) {
  const adapterSlots = [
    'bg',
    'bg-secondary',
    'bg-tertiary',
  ...
  adapterSlots.forEach(slot => {
    const value = vars[`--sparo-${slot}`];
    const legacyRuntimeVar = `${LEGACY_RUNTIME_THEME_PREFIX}${slot}`;
    if (value !== undefined) vars[legacyRuntimeVar] = value;
  });
}
