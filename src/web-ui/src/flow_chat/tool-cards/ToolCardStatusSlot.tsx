...
function StatusIcon({ status, size }: { status: ToolCardStatusSlotStatus; size: DotMatrixLoaderSize }) {
  switch (status) {
    case 'completed':
      return <Check size={size === 'small' ? 12 : 16} className="tcss-check" />;
    case 'error':
      return <X size={size === 'small' ? 12 : 16} className="tcss-error" />;
    case 'cancelled':
      return <X size={size === 'small' ? 12 : 16} className="tcss-cancelled" />;
    default:
      return <DotMatrixLoader size={size} />;
  }
}
