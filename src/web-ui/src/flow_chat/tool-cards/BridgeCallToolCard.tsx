...
export const BridgeCallToolCard: React.FC<ToolCardProps> = React.memo(({ toolItem }) => {
  ...
  const inputData = useMemo(
    () => parseData<BridgeCallInput>(toolCall?.input) ?? {},
    [toolCall?.input],
  );
  const resultData = useMemo(
    () => parseData<BridgeCallResult>(toolResult?.result) ?? {},
    [toolResult?.result],
  );
