...
import { markdownToTopLevelSourceRanges, tiptapDocToTopLevelMarkdownBlocks } from '../tiptap/utils/tiptapMarkdown';
import type { DocPosition, MarkdownScope, MarkdownTarget } from './protocol';
...
export function buildMarkdownTarget(
  ...
  if (scope === 'selection' && selectedText.trim()) {
    ...
    if (absoluteOffset >= 0) {
      return {
        kind: 'selection',
        from: { kind: 'markdownOffset', offset: absoluteOffset },
        to: { kind: 'markdownOffset', offset: absoluteOffset + selectedText.length },
        markdown: selectedText,
      };
    }
  }
