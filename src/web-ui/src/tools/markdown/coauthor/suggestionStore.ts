...
interface SuggestionStoreState {
  ...
  upsertProposal: (proposal: MarkdownEditProposal, status?: ProposalLifecycle) => void;
  ...
  completeProposal: (proposalId: string, options: {
    status: Extract<ProposalLifecycle, 'applied' | 'discarded'>;
    acceptedOpIds?: string[];
    rejectedOpIds?: string[];
  }) => void;
...
export const useSuggestionStore = create<SuggestionStoreState>((set) => ({
  ...
  completeProposal: (proposalId, options) => set((state) => {
    const entry = state.entries[proposalId];
    if (!entry) {
      return state;
    }
    ...
    return {
      entries: {
        ...
        [proposalId]: {
          ...
          acceptedOpIds: Array.from(new Set([
            ...entry.acceptedOpIds,
            ...(options.acceptedOpIds ?? []),
          ])),
          rejectedOpIds: Array.from(new Set([
            ...entry.rejectedOpIds,
            ...(options.rejectedOpIds ?? []),
          ])),
          updatedAt: Date.now(),
        },
      },
