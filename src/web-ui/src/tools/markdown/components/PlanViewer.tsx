...
import { MarkdownEditingSurface } from '../tiptap';
import type { EditorInstance } from '../tiptap';
      ...
      <div className={panelClassName}>
        <div className={toolbarClassName}>
          {isYamlEditingInPanel ? (
            <IconButton
              aria-label={t('markdown.planViewer.toggleYamlEditOff')}
              tooltip={t('markdown.planViewer.toggleYamlEditOff')}
            ...
            <>
              <IconButton
                aria-label={t('markdown.planViewer.toggleYamlEditOn')}
                tooltip={t('markdown.planViewer.toggleYamlEditOn')}
                tooltipPlacement="top"
                size="xs"
                variant="ghost"
          ...
          <div className="yaml-editor-section">
            <div className="yaml-editor-content">
              <MarkdownEditingSurface
                ...
                height="200px"
                width="100%"
                placeholder={t('markdown.planViewer.yamlPlaceholder')}
                readonly={false}
                toolbar={false}
                autofocus={true}
              />
  ...
  if (loading) {
    return (
      <div className="sparo-plan-viewer sparo-plan-viewer--loading">
        <CubeLoading size="medium" text={t('markdown.planViewer.loadingPlan')} />
      </div>
    );
  }
    ...
    <div className="sparo-plan-viewer">
      ...
      >
        <div className="header-left">
          ...
          {hasUnsavedChanges && <Badge variant="warning">{t('markdown.planViewer.unsaved')}</Badge>}
        </div>
        <div className="header-right" onClick={(e) => e.stopPropagation()}>
          {hasTodos && (
            <>
              <Badge variant="neutral">{t('markdown.planViewer.remainingTodos', { count: remainingTodos })}</Badge>
              ...
              <Button
                ...
                loadingLabel={t('markdown.planViewer.building')}
              >
                {buildStatus === 'built' ? (
                  <>
                    <CheckCircle size={14} />
                    <span>{t('markdown.planViewer.built')}</span>
                  </>
                ) : (
                  <span>{t('markdown.planViewer.build')}</span>
                )}
              </Button>
            </>
          )}
      ...
      <div className="plan-viewer-content">
        <div className="plan-markdown">
          <MarkdownEditingSurface
            ...
            height="auto"
            width="100%"
            placeholder={t('markdown.planViewer.contentPlaceholder')}
            readonly={false}
            toolbar={false}
            filePath={filePath}
            basePath={basePath}
