...
import { analyzeMarkdownEditability, type MarkdownEditabilityAnalysis } from '../tiptap/utils/tiptapMarkdown';
...
} from '@/tools/editor/utils/diskFileVersion';
...
const MarkdownEditor: React.FC<MarkdownEditorProps> = ({
  ...
  const previewPlaceholder = isContentEmpty
    ? t('markdown.editor.emptyPreview')
    : t('markdown.editor.placeholder');

  if (loading) {
    return (
      <div className={`sparo-markdown-editor-loading ${className}`}>
        <CubeLoading size="medium" text={t('markdown.editor.loadingFile')} />
      </div>
    );
  }
