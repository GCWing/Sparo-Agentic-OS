...
export async function loadLocalImages(container: HTMLElement): Promise<void> {
  ...
  const loadPromises = Array.from(localImages).map(async (img) => {
    ...
    try {
      ...
      img.alt = i18nService.t('tools:markdown.tiptap.localImageAltLoadFailedWithSrc', { src: originalSrc });
      img.title = i18nService.t('tools:markdown.tiptap.localImageTitleUnableToLoadWithPath', { path: localPath });
    }
  });
