...
export type DocumentScope = 'selection' | 'block' | 'document';
export type DocumentIntent = 'apply' | 'review';
...
export type DocumentEditProposalChunk =
  | { type: 'proposal'; proposal: DocumentEditProposal }
...
export interface DocumentAction {
  ...
  group?: string;
  icon?: string;
  targets: DocumentScope[];
  modes: DocumentIntent[];
  inputSchema?: unknown;
  shortcut?: string;
...
function parseOp(value: unknown): DocumentEditOp | null {
...
export function parseDocumentEditProposal(value: unknown): DocumentEditProposal {
  if (!isRecord(value)) {
  ...
  const ops = Array.isArray(value.ops) ? value.ops.map(parseOp).filter((op): op is DocumentEditOp => !!op) : [];
...
export function extractProposalFromText(text: string): DocumentEditProposal {
  const trimmed = text.trim();
  const fenced = trimmed.match(/```(?:json)?\s*([\s\S]*?)```/i);
  const candidate = fenced?.[1]?.trim() ?? trimmed;
  return parseDocumentEditProposal(JSON.parse(candidate));
}
