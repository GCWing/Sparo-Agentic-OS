...
import type { DocumentEditOp, DocumentEditProposal } from './protocol';
...
export interface PersistedCoauthorComment {
  ...
  filePath?: string;
  sourceHash: string;
  from: Extract<DocumentEditOp, { type: 'comment' }>['from'];
  to: Extract<DocumentEditOp, { type: 'comment' }>['to'];
  message: string;
...
export async function persistAcceptedComments(
  ...
  const comments = proposal.ops.filter((op): op is Extract<DocumentEditOp, { type: 'comment' }> => (
