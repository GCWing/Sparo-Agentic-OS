...
export const Avatar: React.FC<AvatarProps> = ({
  ...
  const avatarClass = [
    'ds-avatar',
    `ds-avatar--${shape}`,
    typeof size === 'string' && `ds-avatar--${size}`,
    className
  ].filter(Boolean).join(' ');
  ...
  const renderContent = () => {
    ...
    if (icon) {
      return <span className="ds-avatar__icon">{icon}</span>;
    }
    if (children) {
      return <span className="ds-avatar__text">{children}</span>;
    }
    return null;
  };
...
export const AvatarGroup: React.FC<AvatarGroupProps> = ({
  ...
  return (
    <div className={`ds-avatar-group ${className}`} style={style}>
      {displayChildren}
      {restCount > 0 && (
        <Avatar className="ds-avatar-group__rest">+{restCount}</Avatar>
      )}
    </div>
  );
};
