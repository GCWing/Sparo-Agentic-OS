...
use bitfun_core::agentic::tools::computer_use_host::{
    ...
    UiElementLocateQuery, UiElementLocateResult, VisualActionResult, VisualClickParams,
...
impl DesktopComputerUseHost {
    ...
    /// / agent goes blind" symptom we observed against the Sparo OS WebView.
    fn run_background_input_self_check(&self) {
        #[cfg(target_os = "macos")]
        {
            ...
            if bg_ok {
                ...
                // permission lands. Subsequent Sparo OS launches skip the
    ...
    fn permission_sync() -> ComputerUsePermissionSnapshot {
        ...
        #[cfg(target_os = "macos")]
        {
            let platform_note = if cfg!(debug_assertions) && !macos::ax_trusted() {
                Some(
                    "Development build: grant Accessibility to target/debug/sparo-os (path appears in errors if mouse fails)."
                        .to_string(),
                )
            } else {
                None
    ...
    pub(crate) async fn get_app_state_inner(
        ...
        #[cfg(target_os = "macos")]
        {
            ...
            let mut snap = tokio::task::spawn_blocking(move || {
                ...
                // unwound foreign exception aborts the whole Sparo OS process
    ...
    async fn app_click(&self, params: AppClickParams) -> BitFunResult<AppStateSnapshot> {
        #[cfg(target_os = "macos")]
        {
            ...
            if !ax_ok {
                ...
                // unchanged AND the target is our own process (sparo-os
...
async fn resolve_pid_macos(host: &DesktopComputerUseHost, app: &AppSelector) -> BitFunResult<i32> {
    ...
    if let Some(name) = app.name.as_deref() {
        ...
        //    asks for "Sparo OS" but `list_apps` returned name="sparo-os"
        //    with bundle_id="com.sparo-os.desktop"). This keeps us aligned with
