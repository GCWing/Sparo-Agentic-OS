...
use sparo_core::agentic::tools::computer_use_host::{UiElementLocateQuery, UiElementLocateResult};
use sparo_core::error::{CoreError, CoreResult};
use screenshots::display_info::DisplayInfo;

pub fn validate_query(q: &UiElementLocateQuery) -> CoreResult<()> {
...
fn global_xy_to_native_with_display(d: &DisplayInfo, gx: f64, gy: f64) -> CoreResult<(u32, u32)> {
...
pub fn global_to_native_center(gx: f64, gy: f64) -> CoreResult<(u32, u32)> {
    let d = DisplayInfo::from_point(gx.round() as i32, gy.round() as i32)
        .map_err(|e| CoreError::tool(format!("DisplayInfo::from_point: {}", e)))?;
