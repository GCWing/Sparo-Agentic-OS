...
impl GlobalConfigManager {
    ...
    pub async fn initialize() -> CoreResult<()> {
        ...
        CONFIG_UPDATE_SENDER.set(sender).map_err(|_| {
            CoreError::config("Failed to initialize config update sender".to_string())
        })?;
    ...
    pub async fn update_service(new_service: Arc<ConfigService>) -> CoreResult<()> {
        let service_wrapper = GLOBAL_CONFIG_SERVICE.get().ok_or_else(|| {
            CoreError::config("Global config service not initialized".to_string())
        })?;
    ...
    pub async fn reload() -> CoreResult<()> {
    ...
    pub async fn update_theme(&self, theme_id: &str) -> CoreResult<()> {
        let service = Self::get_service().await?;
...
pub async fn reload_global_config() -> CoreResult<()> {
