...
pub struct SenseVoiceInt8Recognizer {
...
impl SpeechRecognizer for SenseVoiceInt8Recognizer {
    async fn warmup(&self, model_dir: PathBuf, language: String) -> CoreResult<()> {
        ...
        tokio::task::spawn_blocking(move || {
            ...
            let mut cache = cache
                .lock()
                .map_err(|_| CoreError::service("Speech recognizer cache lock is poisoned"))?;
            ensure_cached_recognizer(&mut cache, model_path, tokens_path, language)?;
            Ok(())
        })
        .await
        .map_err(|e| CoreError::service(format!("Speech recognizer warmup task failed: {e}")))?
    }
    ...
    async fn unload(&self) -> CoreResult<()> {
        ...
        .await
        .map_err(|e| CoreError::service(format!("Speech recognizer unload task failed: {e}")))?
    }
    ...
    async fn transcribe(
        ...
        tokio::task::spawn_blocking(move || transcribe_blocking(request, cache))
            .await
            .map_err(|e| CoreError::service(format!("Speech transcription task failed: {e}")))?
    }
}
...
fn transcribe_blocking(
    ...
    if samples.is_empty() {
        return Err(CoreError::validation("No audio samples were provided"));
    }
    ...
    let text = {
        ...
        stream
            .get_result()
            .ok_or_else(|| CoreError::service("Failed to read speech result"))?
            .text
            .trim()
            .to_string()
    };
...
fn ensure_model_files(model_path: &Path, tokens_path: &Path) -> CoreResult<()> {
...
fn ensure_cached_recognizer(
    ...
    tokens_path: PathBuf,
    language: String,
) -> CoreResult<&mut CachedSenseVoiceRecognizer> {
    ...
    cache
        .as_mut()
        .ok_or_else(|| CoreError::service("Speech recognizer cache is empty"))
}
...
fn create_recognizer(
    ...
    OfflineRecognizer::create(&config)
        .ok_or_else(|| CoreError::service("Failed to create speech recognizer"))
}
