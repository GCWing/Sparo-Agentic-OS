...
use super::catalog::{
    ...
    AppInstallScope, AppInteractionModel, AppSurfaceMode, AppTruthSource, AppWorkMultiplicity,
    CapabilityRef, ComponentDefinition, ComponentKind, ComponentOwnerApp, ComponentPackageSource,
    ComponentSource, ComponentVisibility, PermissionSpec, ProductAppLaunch, ProductAppLaunchKind,
...
pub async fn create_product_app_package_with_options(
    ...
    if let Some(primary_surface_definition) = primary_surface_definition.as_ref() {
        ...
        .await?;
        write_private_surface_source(&primary_surface_dir, &draft.name, &draft.goal).await?;
    }
        ...
        ComponentKind::Agent => {
            ...
            write_text(
                ...
                &format!(
                    ...
                    component.name, app.id, app.goal
                ),
            )
            .await?;
            ...
            write_json(
                ...
                &json!({
                    ...
                    "fixtures": [
                        {
                            "id": "primary-behavior",
                            "input": { "message": format!("Help with: {}", app.goal) },
                            "expect": ["goal-specific response", "actionable next step"]
                        }
                    ]
                }),
            )
...
fn default_product_app_validation_plan(
    ...
    let preview_gate = if include_surface {
        "- `RunStudioPreview` with `mode=\"product-app-preview\"`: primary surface identity and placement resolve before claiming UI readiness."
    } else {
        "- `RunStudioPreview` with `mode=\"agent-chat\"`: the app-private Agent entry resolves before claiming conversational readiness."
    };
    let runtime_boundary_gate = if include_surface {
        "- `RunStudioPreview` with `mode=\"runtime-boundary\"`: Work id, runtime instance id, Product App version, component lock digest, and primary surface id are all bound to the same Product App runtime context; data behavior, retention, deletion, migration, and share impact still require runtime evidence."
    } else {
        "- `RunStudioPreview` with `mode=\"runtime-boundary\"`: Work/session id, Product App version, component lock digest, and Agent entry are bound to the same Product App runtime context; data behavior, retention, deletion, migration, and share impact still require runtime evidence."
    };
    let agent_eval_gate = agent_component_id
        .map(|component_id| {
            format!(
                "- `RunStudioPreview` with `mode=\"agent-eval\"` and `execute=true`: required eval cases in `tests/eval.json` produce evidence according to `evidenceKind`. `runtime-binding` cases prove agent runtime binding readiness for `{component_id}`; `behavior` cases use the App Studio hidden-agent runner when parent session, turn, workspace, and coordinator context are available; otherwise they remain `notVerified`."
            )
        })
        .unwrap_or_else(|| {
            "- Agent Eval is not required for the initial surface-only package. If intelligent backend behavior or `permissions.ai` is added later, add an app-private Agent Component, create `tests/eval.json`, and run `RunStudioPreview` with `mode=\"agent-eval\"`.".to_string()
        });
...
{}
{}
- `RunStudioPreview` with `mode="runtime-dependencies"`: Product App Runtime host records source loading, import-map/CDN resolution when used, Node/dependency install state when used, and worker freshness before dependency health can pass.
- `RunStudioPreview` with `mode="permission-review"`: App Studio records explicit permission review evidence; package permission declarations alone do not pass the review gate.
- `RunStudioPreview` with `mode="user-path-rehearsal"`: the critical user path in `tests/rehearsal.json` is observed, not inferred from package metadata.
{}
- `RunStudioPreview` with `mode="release-rehearsal"`: release readiness only aggregates the independent gates above and must remain `notVerified` while any gate lacks evidence.
    ...
    ProductAppRehearsalPlan {
        ...
        scenarios: vec![
            ProductAppRehearsalScenario {
                ...
                description: format!(
                    "Open the app and observe the path that should make progress toward: {}",
                    draft.goal
                ),
                ...
                steps: vec![
                    ...
                    ProductAppRehearsalStep {
                        ...
                        value: None,
                        expect: vec![focus_expectation.to_string(), draft.goal.clone()],
                    },
                    ProductAppRehearsalStep {
                        id: "observe-goal-progress".to_string(),
                        action: ProductAppRehearsalAction::Observe,
                        target: Some("work-outcome".to_string()),
                        value: None,
                        expect: vec![format!("Progress target: {}", draft.goal)],
                    },
                ],
                expected: vec![
                    "The app opens without fatal runtime issues.".to_string(),
                    "The primary workflow has an observable path toward the Product App goal."
                        .to_string(),
    ...
    ProductAppEvalPlan {
        ...
        cases: vec![
            ...
            ProductAppEvalCase {
                ...
                description: format!(
                    "Representative Product App eval for the core goal: {}",
                    app_goal
                ),
                component_id: Some(agent_component_id.to_string()),
                implementation_ref: Some(format!("agent://{}", agent_type)),
                ...
                tool_name: None,
                input: json!({
                    "message": format!("Help with: {}", app_goal),
                    "goal": app_goal,
                    "appId": app_id,
                    "expectedEvidence": [
...
if (root) {{
  root.innerHTML = `
    ...
    <p class="product-app-surface__outcome" data-testid="work-outcome">Progress target: ${{appGoal}}</p>
  `;
  ...
  if (action && outcome) {{
    action.addEventListener('click', () => {{
      outcome.textContent = `Progress target: ${{appGoal}}`;
    }});
  }}
}}
"#
...
mod tests {
    ...
    async fn creates_resolved_agent_only_product_app_package_with_lock() {
        ...
        assert_eq!(launch.target_id, "agentic");
    ...
    async fn creates_app_private_component_scaffolds_for_all_kinds() {
        ...
        let written = create_product_app_package(
            ...
            CreateProductAppPackageDraft {
                ...
                goal: "Coordinate several app-private implementation units".to_string(),
                version: "1.0.0".to_string(),
                agent_type: "agentic".to_string(),
                category: "testing".to_string(),
                tags: Vec::new(),
