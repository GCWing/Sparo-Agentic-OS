...
impl JsWorkerPool {
    ...
    pub async fn get_or_spawn(
        ...
        if let Some(entry) = guard.remove(worker_id) {
            if entry.revision == worker_revision {
                let worker = Arc::clone(&entry.worker);
                guard.insert(worker_id.to_string(), entry);
                return Ok(worker);
            }
    ...
    pub fn has_installed_deps(&self, app_id: &str) -> bool {
        self.path_manager
            .product_app_runtime_host_dir(app_id)
            .join("node_modules")
            .exists()
    }
    ...
    pub async fn install_deps(
        ...
        let (cmd, args): (&str, &[&str]) = match self.runtime.kind {
            crate::product_app_runtime_host_engine::runtime_detect::RuntimeKind::Bun => {
                ("bun", &["install", "--production"][..])
            }
            crate::product_app_runtime_host_engine::runtime_detect::RuntimeKind::Node => {
