...
/// Export engine: check prerequisites and export a Product App runtime host to a standalone app.
pub struct ProductAppRuntimeHostExporter {
    #[allow(dead_code)]
...
impl ProductAppRuntimeHostExporter {
    pub fn new(
        path_manager: Arc<crate::infrastructure::PathManager>,
        templates_dir: PathBuf,
    ) -> Self {
    ...
    pub async fn check(&self, _app_id: &str) -> CoreResult<ExportCheckResult> {
        ...
        let runtime_str = runtime.as_ref().map(|r| {
            match r.kind {
                crate::product_app_runtime_host_engine::runtime_detect::RuntimeKind::Bun => "bun",
                crate::product_app_runtime_host_engine::runtime_detect::RuntimeKind::Node => "node",
            }
            .to_string()
        });
