...
use crate::product_app_runtime_host_engine::manager::try_get_global_product_app_runtime_host_manager;
use crate::product_app_runtime_host_engine::runtime_detect::DetectedRuntime;
...
fn parse_log_level(value: Option<&str>) -> ProductAppRuntimeHostRuntimeLogLevel {
    match value {
        Some("debug") => ProductAppRuntimeHostRuntimeLogLevel::Debug,
        Some("warn") => ProductAppRuntimeHostRuntimeLogLevel::Warn,
        Some("error") => ProductAppRuntimeHostRuntimeLogLevel::Error,
        _ => ProductAppRuntimeHostRuntimeLogLevel::Info,
    }
}

fn parse_worker_log_line(app_id: &str, line: &str) -> ProductAppRuntimeHostRuntimeLog {
    let parsed = serde_json::from_str::<Value>(line).ok();
    if let Some(value) = parsed.as_ref().and_then(Value::as_object) {
        return ProductAppRuntimeHostRuntimeLog {
    ...
    pub async fn spawn(
        ...
        tokio::spawn(async move {
            ...
            while let Ok(Some(line)) = lines.next_line().await {
                ...
                if let Some(event_name) = msg.get("event").and_then(Value::as_str) {
                    ...
                    let event_full_name =
                        format!("product-app-runtime://worker-event:{}", stderr_app_id);
        ...
        tokio::spawn(async move {
            ...
            while let Ok(Some(line)) = lines.next_line().await {
                ...
                if let Some(manager) = try_get_global_product_app_runtime_host_manager() {
