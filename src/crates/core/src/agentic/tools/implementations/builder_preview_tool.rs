...
pub struct RunBuilderPreviewTool;

impl RunBuilderPreviewTool {
    pub fn new() -> Self {
        Self
    }
}

impl Default for RunBuilderPreviewTool {
    fn default() -> Self {
        Self::new()
    }
}
...
#[async_trait]
impl Tool for RunBuilderPreviewTool {
    fn name(&self) -> &str {
        "RunBuilderPreview"
    }
        ...
        json!({
            ...
            "properties": {
                "mode": {
                    ...
                    "description": "Preview Harness mode. Defaults to auto from the bound App Builder subject."
                },
                "intent": {
                    "type": "string",
            ...
            "description": "Leave input empty to run the default preview harness for the current bound AppBuilder subject."
        })
    }
    ...
    async fn call_impl(
        ...
        let Some(app_builder) = context.app_builder.as_ref() else {
            return Err(CoreError::validation(
                "RunBuilderPreview requires a bound AppBuilder session".to_string(),
            ));
        };

        let requested_mode = optional_string(input, "mode").unwrap_or_else(|| "auto".to_string());
        let mode = resolve_harness_mode(&requested_mode, app_builder).await;
        ...
        let target = target_summary(app_builder);
        ...
        let mut checks = vec![
            check(
                "target",
                "passed",
                format!("Bound App Builder target resolved: {target}."),
            ),
            check(
                "fixture",
                ...
                if fixture_provided {
                    ...
                    if mode == "agent-eval" {
                        ...
                        "No explicit fixture input was provided; this harness consumes the latest App Builder Workbench evidence for the bound Product App Work."
                            .to_string()
                    } else {
        ...
        let runtime_instance_id = app_builder.runtime_instance_id.clone();
        match mode.as_str() {
            "agent-chat" => {
                checks
                    .extend(agent_chat_preview_checks(app_builder, context, input, execute).await);
            }
            "product-app-preview" | "sidecar" | "full-app" | "embedded" => {
                checks.extend(
                    work_preview_observation_checks(
                        app_builder,
                        context,
                        &mode,
                        "host observation",
            ...
            "agent-eval" => {
                checks
                    .extend(agent_eval_preview_checks(app_builder, context, input, execute).await);
            }
            "runtime-boundary" => {
                ...
                if evidence.found {
                    ...
                    checks.extend(runtime_boundary_preview_checks(app_builder).await);
                    checks.extend(evidence.checks);
                }
            }
            "runtime-dependencies" => {
                ...
                if evidence.found {
                    ...
                    checks.extend(runtime_dependency_preview_checks(app_builder).await);
                    checks.extend(evidence.checks);
                }
            }
            "permission-review" => {
                ...
                if evidence.found {
                    ...
                    checks.extend(permission_review_preview_checks(app_builder).await);
                    checks.extend(evidence.checks);
                }
            }
            "user-path-rehearsal" => {
                ...
                if evidence.found {
                    ...
                    checks.extend(user_path_rehearsal_preview_checks(app_builder).await);
                    checks.extend(evidence.checks);
                }
            }
            "release-rehearsal" => {
...
fn agent_chat_preview_prompt(
    ...
    format!(
        "Preview Product App {}@{}. Description: {}. Reply with one concrete next step this app can take for a user.",
        app_id,
        version,
        ...
        && skill_id.is_none()
    {
        match (interactive_surface_runtime_id, selected_action.as_deref()) {
            (Some(runtime_id), Some(action)) => Some(resolve_interactive_surface_runtime_binding(
                ...
                &component,
                action,
                app_builder.runtime_instance_id.as_deref(),
            )),
            _ => None,
        }
    } else {
        ...
        check(
            ...
            if let Some(Ok(result)) = run_result.as_ref() {
                ...
                if binding.selected {
                    ...
                    format!(
                        "Skill binding skill://{} exists as {}, but is not selected for the current App Builder agent/workspace runtime.",
...
async fn agent_eval_product_app_checks(
    ...
    let package = match ProductAppResolver::read_product_app_package(&app_builder.package_root)
        .await
    {
        Ok(package) => package,
        Err(error) => {
    ...
    if !execute {
        return vec![
            check(
                ...
                "notVerified",
                format!(
                    "Product App {app_id}@{version} declares {} Agent Eval case(s) in tests/eval.json. Run RunBuilderPreview with mode=\"agent-eval\" and execute=true to execute them.",
                    cases.len()
                ),
            ),
            check(
...
fn behavior_eval_prompt(case: &ProductAppEvalCase) -> String {
    ...
    format!(
        "Run this App Builder behavior eval case.\n\nCase: {}\nTitle: {}\nDescription: {}\nInput JSON: {}",
        case.id,
        case.title,
        case.description,
        case.input
    ...
    if !execute {
        return vec![
            check(
                ...
                "notVerified",
                format!(
                    "{required_detail} Run RunBuilderPreview with mode=\"agent-eval\", execute=true, and a representative fixture/action to produce executable evidence."
                ),
            ),
            check(
                "evalLogs",
...
fn resolve_interactive_surface_runtime_binding(
    ...
    if runtime_id != "interactive-surface" {
        return Err(format!(
            "runtime://{runtime_id} is not a registered App Builder runtime executor"
        ));
    }
...
async fn runtime_boundary_product_app_checks(
    ...
    let runtime_instance = app_builder
        .runtime_instance_id
        .as_deref()
...
async fn permission_review_product_app_checks(
    ...
    vec![
        ...
        check(
            ...
            "notVerified",
            if permission_names.is_empty() {
                "No elevated permissions are declared, but an explicit App Builder permission review has not been recorded."
                    .to_string()
            } else {
                "Elevated permissions require explicit review evidence recorded by the App Builder host after runtime permission summary is available."
                    .to_string()
            },
        ),
    ]
    ...
    vec![
        ...
        check(
            ...
            if permission_names.is_empty() {
                ...
                "Declared Product App permissions require explicit App Builder review evidence before release."
                    .to_string()
            },
        ),
        check(
...
fn mode_to_work_preview_kind(mode: &str) -> WorkBuilderPreviewKind {
    match mode {
        "agent-chat" => WorkBuilderPreviewKind::AgentChat,
        "sidecar" => WorkBuilderPreviewKind::Sidecar,
        "full-app" => WorkBuilderPreviewKind::FullApp,
        "embedded" => WorkBuilderPreviewKind::Embedded,
        "capability" => WorkBuilderPreviewKind::Capability,
        "agent-eval" => WorkBuilderPreviewKind::AgentEval,
        "runtime-boundary" => WorkBuilderPreviewKind::RuntimeBoundary,
        "runtime-dependencies" => WorkBuilderPreviewKind::RuntimeDependencies,
        "permission-review" => WorkBuilderPreviewKind::PermissionReview,
        "user-path-rehearsal" => WorkBuilderPreviewKind::UserPathRehearsal,
        "release-rehearsal" => WorkBuilderPreviewKind::ReleaseRehearsal,
        _ => WorkBuilderPreviewKind::ProductAppPreview,
    }
}
...
async fn work_preview_observation_checks(
    ...
    let runtime_instance_id = app_builder.runtime_instance_id.as_deref();
    let runtime_identity_required = mode != "permission-review";

    let Some(work_id_text) = app_builder.work_id.as_deref() else {
        ...
        checks.push(check(
            ...
            "notVerified",
            format!(
                "No Work id is bound to this App Builder turn; {evidence_label} cannot be read from the Work graph."
            ),
        ));
        checks.push(check(
            "previewObservation",
            "notVerified",
            format!(
                "Concrete {evidence_label} belongs to the App Builder Workbench or Product App Runtime host; no bound Work graph was available for this tool to read the latest observation."
            ),
        ));
    ...
    let work_id = match WorkId::parse(work_id_text.to_string()) {
        ...
        Err(error) => {
            ...
            checks.push(check(
                "previewObservation",
                "notVerified",
                format!("Bound App Builder Work id is invalid: {error}."),
            ));
            return WorkPreviewEvidence {
                checks,
                found: false,
...
fn runtime_identity_check(
    ...
    check(
        ...
        if let Some(id) = bound_runtime_instance_id {
            format!("Runtime instance {id} is bound to this App Builder turn.")
        } else if let Some(id) = observed_runtime_instance_id {
            format!("Runtime instance {id} is recorded by the Workbench observation.")
        } else if required {
            "No runtime instance is bound to this App Builder turn or recorded by the Workbench observation."
                .to_string()
        } else {
...
fn work_preview_check_to_json(check: &crate::agentic_os::work::WorkBuilderFactCheck) -> Value {
    json!({
        "id": check.id.clone(),
...
fn preview_status_for_check(status: WorkBuilderFactStatus) -> &'static str {
    match status {
        ...
        WorkBuilderFactStatus::Running | WorkBuilderFactStatus::Waiting => "running",
        WorkBuilderFactStatus::NotRun => "notRun",
        WorkBuilderFactStatus::NotVerified | WorkBuilderFactStatus::Ready => "notVerified",
    }
}

fn preview_status_string(status: WorkBuilderFactStatus) -> &'static str {
    match status {
        WorkBuilderFactStatus::Passed => "passed",
        WorkBuilderFactStatus::Warning => "warning",
        WorkBuilderFactStatus::Failed => "failed",
...
"#
    ...
    #[tokio::test]
    async fn run_builder_preview_requires_bound_context() {
    ...
    async fn agent_eval_product_app_requires_executable_evidence() {
        let tool = RunBuilderPreviewTool::new();
        let base = std::env::temp_dir().join(format!(
            "sparo-run-builder-preview-product-agent-eval-{}",
            uuid::Uuid::new_v4()
        ));
        let path_manager = PathManager::with_user_root_for_tests(base.clone());
        let written = create_product_app_package_with_options(
    ...
    async fn agent_eval_product_app_runs_package_eval_plan_js_tool_case() {
        ...
        let tool = RunBuilderPreviewTool::new();
        let base = std::env::temp_dir().join(format!(
            "sparo-run-builder-preview-product-agent-eval-plan-{}",
            uuid::Uuid::new_v4()
        ));
    ...
    async fn concrete_release_readiness_modes_report_independent_preview_kinds() {
        let tool = RunBuilderPreviewTool::new();
        let base = std::env::temp_dir().join(format!(
            "sparo-run-builder-preview-readiness-modes-{}",
            uuid::Uuid::new_v4()
        ));
        let path_manager = PathManager::with_user_root_for_tests(base.clone());
        let written = create_product_app_package_with_options(
    ...
    async fn runtime_dependencies_with_browser_esm_dependency_require_runtime_evidence() {
        let tool = RunBuilderPreviewTool::new();
        let base = std::env::temp_dir().join(format!(
            "sparo-run-builder-preview-runtime-deps-{}",
            uuid::Uuid::new_v4()
        ));
        let path_manager = PathManager::with_user_root_for_tests(base.clone());
        let written = create_product_app_package_with_options(
    ...
    async fn release_rehearsal_product_app_reads_package_permission_data_eval_evidence() {
        let tool = RunBuilderPreviewTool::new();
        let base = std::env::temp_dir().join(format!(
            "sparo-run-builder-preview-product-{}",
            uuid::Uuid::new_v4()
        ));
        let path_manager = PathManager::with_user_root_for_tests(base.clone());
        let written = create_product_app_package_with_options(
    ...
    async fn release_rehearsal_surface_only_product_app_does_not_require_agent_eval() {
        let tool = RunBuilderPreviewTool::new();
        let base = std::env::temp_dir().join(format!(
            "sparo-run-builder-preview-surface-only-product-{}",
            uuid::Uuid::new_v4()
        ));
        let path_manager = PathManager::with_user_root_for_tests(base.clone());
