...
pub struct AppStudioAgent {
    default_tools: Vec<String>,
}

impl Default for AppStudioAgent {
    fn default() -> Self {
        Self::new()
    }
}
    ...
    pub fn new() -> Self {
        Self {
            default_tools: vec![
                ...
                "ResolveStudioPreviewTarget".to_string(),
                "CreateProductAppCheckpoint".to_string(),
                "CompareProductAppRevisions".to_string(),
                "CreateProductAppFromReleaseTemplate".to_string(),
                "RestoreProductAppCheckpoint".to_string(),
                "RestoreProductAppRelease".to_string(),
                "CreateProductAppRelease".to_string(),
                "PublishProductAppRelease".to_string(),
                "ValidateProductAppPackage".to_string(),
                "RunStudioPreview".to_string(),
...
impl Agent for AppStudioAgent {
    ...
    fn description(&self) -> &str {
        "App Studio: design, debug, and evolve Product Apps from a user goal"
    }
...
#[cfg(test)]
mod tests {
    use super::{Agent, AppStudioAgent};

    #[test]
    fn default_tools_are_focused_on_product_app_delivery() {
        let agent = AppStudioAgent::new();
    ...
    fn default_tools_exclude_broad_or_destructive_surfaces() {
        let tools = AppStudioAgent::new().default_tools();
        ...
        for excluded_tool in [
            ...
            assert!(
                ...
                "{excluded_tool} should not be a default App Studio tool"
            );
        }
    }
}
