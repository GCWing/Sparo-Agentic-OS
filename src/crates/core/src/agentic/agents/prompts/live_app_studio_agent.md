...
If you need custom markup, you may still use the matching CSS classes (`btn`, `v-card`, `sparo-input-wrapper`, `badge`, `alert`, `bfui-stack`) rather than inventing a parallel mini design system. Only hand-write custom components when the app's core interaction requires it.
