...
pub fn seed_builtin_bridge_apps() -> BitFunResult<()> {
    ...
    for bundle_path in collect_builtin_bundle_paths() {
        if let Err(error) = seed_bundle_by_path(&bundle_path) {
            log::warn!(
                "seed builtin bridge app bundle '{}' failed: {}",
                bundle_path.display(),
                error
            );
        }
...
pub fn ensure_builtin_bridge_app_current(app_id: &str) -> BitFunResult<bool> {
    for bundle_path in collect_builtin_bundle_paths() {
...
fn collect_builtin_bundle_paths() -> Vec<PathBuf> {
    let mut paths: BTreeSet<PathBuf> = BUILTIN_BRIDGE_APPS_DIR
        .dirs()
...
fn embedded_bundle_dir(relative_bundle_dir: &Path) -> Option<&'static Dir<'static>> {
    ...
    BUILTIN_BRIDGE_APPS_DIR.get_dir(normalized.as_str())
}
...
fn seed_one(bundle_dir: &Dir<'_>) -> BitFunResult<()> {
    ...
    log::info!(
        "seeded builtin bridge app '{}' (v{}, source {})",
        bundle.id,
        bundle.version,
        source_digest
    );
...
) -> BitFunResult<BuiltinBridgeAppBundle> {
    let bundle_dir = filesystem_bundles_root().join(relative_bundle_dir);
