//! Surface Component storage: persist and load under user data dir.

use crate::surface_component::types::{
    NpmDep, SurfaceComponent, SurfaceComponentEntry, SurfaceComponentMeta, SurfaceComponentSource,
    SurfaceComponentSourceFile, SurfaceComponentSourceFileKind,
};
...
fn infer_source_file_kind(path: &str) -> SurfaceComponentSourceFileKind {
    if path.ends_with(".js") || path.ends_with(".mjs") || path.ends_with(".ts") {
        SurfaceComponentSourceFileKind::Script
    } else if path.ends_with(".css") || path.ends_with(".scss") {
        SurfaceComponentSourceFileKind::Style
    } else if path.ends_with(".html") {
        SurfaceComponentSourceFileKind::Html
    } else if path.ends_with(".json") {
        SurfaceComponentSourceFileKind::Json
    } else {
        SurfaceComponentSourceFileKind::Asset
    }
}
...
/// Surface Component storage service (file-based under `path_manager.surface_components_dir()`).
pub struct SurfaceComponentStorage {
    path_manager: Arc<crate::infrastructure::PathManager>,
}

impl SurfaceComponentStorage {
    pub fn new(path_manager: Arc<crate::infrastructure::PathManager>) -> Self {
        Self { path_manager }
    }

    fn app_dir(&self, app_id: &str) -> PathBuf {
        self.path_manager.surface_component_dir(app_id)
    }
    ...
    /// List all app IDs (directories under `surface_components_dir`).
    pub async fn list_app_ids(&self) -> BitFunResult<Vec<String>> {
        ...
        let mut read_dir = tokio::fs::read_dir(&root).await.map_err(|e| {
            BitFunError::io(format!("Failed to read surface_components dir: {}", e))
        })?;
        while let Some(entry) = read_dir.next_entry().await.map_err(|e| {
            BitFunError::io(format!("Failed to read surface_components entry: {}", e))
        })? {
            let path = entry.path();
            if path.is_dir() {
    ...
    /// Load full Surface Component by id (meta + source + compiled_html).
    pub async fn load(&self, app_id: &str) -> BitFunResult<SurfaceComponent> {
        let meta_path = self.meta_path(app_id);
        let meta_content = tokio::fs::read_to_string(&meta_path).await.map_err(|e| {
            if e.kind() == std::io::ErrorKind::NotFound {
                BitFunError::NotFound(format!("Surface Component not found: {}", app_id))
            } else {
                BitFunError::io(format!("Failed to read meta: {}", e))
            }
        })?;
        let meta: SurfaceComponentMeta = serde_json::from_str(&meta_content)
    ...
    pub async fn load_meta(&self, app_id: &str) -> BitFunResult<SurfaceComponentMeta> {
        let meta_path = self.meta_path(app_id);
    ...
    async fn load_source(&self, app_id: &str) -> BitFunResult<SurfaceComponentSource> {
    ...
    async fn load_source_entry(&self, app_id: &str) -> BitFunResult<SurfaceComponentEntry> {
        let path = self.source_dir(app_id).join(SOURCE_MANIFEST_JSON);
        if !path.exists() {
            return Ok(SurfaceComponentEntry::default());
        }
    ...
    async fn load_extra_source_files(
        &self,
        app_id: &str,
    ) -> BitFunResult<Vec<SurfaceComponentSourceFile>> {
        let sd = self.source_dir(app_id);
        ...
        while let Some(dir) = stack.pop() {
                ...
                .map_err(|e| BitFunError::io(format!("Failed to read source entry: {}", e)))?
            {
                ...
                files.push(SurfaceComponentSourceFile {
    ...
    pub async fn load_source_only(&self, app_id: &str) -> BitFunResult<SurfaceComponentSource> {
    ...
    /// Save full Surface Component (meta, source files, compiled.html).
    pub async fn save(&self, app: &SurfaceComponent) -> BitFunResult<()> {
        self.ensure_app_dir(&app.id).await?;

        let meta = SurfaceComponentMeta::from(app);
    ...
    /// Delete Surface Component directory entirely.
    ...
    pub async fn load_version(&self, app_id: &str, version: u32) -> BitFunResult<SurfaceComponent> {
