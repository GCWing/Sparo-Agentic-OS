//! Surface Component compiler 鈥?assemble source (html/css/ui_js) + Import Map + Runtime Adapter + CSP into compiled_html.

use crate::surface_component::bridge_builder::{
    build_bridge_script, build_csp_content, build_import_map,
    build_surface_component_default_theme_css, preview_element_inspector_script,
    scroll_boundary_script,
};
use crate::surface_component::runtime_ui_kit::{
    build_runtime_ui_kit_css, build_runtime_ui_kit_script,
};
use crate::surface_component::types::{
    SurfaceComponentBuildMode, SurfaceComponentPermissions, SurfaceComponentSource,
...
const EMBEDDED_ESM_SPECIFIER_PREFIX: &str = "sparo-surface-component/";

/// Compile Surface Component source into full HTML with Import Map, Runtime Adapter, and CSP injected.
pub fn compile(
    ...
    let theme_default_style = build_surface_component_default_theme_css();
...
fn strip_compiled_entry_scripts(source: &SurfaceComponentSource) -> String {
    match source.entry.build_mode {
        SurfaceComponentBuildMode::NativeEsm | SurfaceComponentBuildMode::Bundled => {
...
fn build_user_script_tag(source: &SurfaceComponentSource) -> BitFunResult<String> {
    match source.entry.build_mode {
        SurfaceComponentBuildMode::NativeEsm | SurfaceComponentBuildMode::Bundled => {
...
fn build_style_content(source: &SurfaceComponentSource) -> String {
...
fn resolve_ui_entry_code(source: &SurfaceComponentSource, entry: &str) -> BitFunResult<String> {
    ...
    source
        ...
        .ok_or_else(|| {
            BitFunError::validation(format!("Surface Component UI entry not found: {}", entry))
        })
}
...
fn build_embedded_esm_entry(
    ...
    let mut modules: BTreeMap<String, String> = source
        ...
        .iter()
        .filter(|file| {
            file.kind == SurfaceComponentSourceFileKind::Script
...
mod tests {
    ...
    fn compile_injects_runtime_ui_kit() {
        let source = SurfaceComponentSource {
        ...
        let html = compile(
            &source,
            &SurfaceComponentPermissions::default(),
            "app-id",
            "/tmp/appdata",
            "/tmp/workspace",
            "dark",
    ...
    fn compile_native_esm_embeds_import_map_and_removes_dev_script() {
        let source = SurfaceComponentSource {
            ...
            worker_js: String::new(),
            npm_dependencies: Vec::new(),
            entry: crate::surface_component::types::SurfaceComponentEntry {
            ...
            source_files: vec![crate::surface_component::types::SurfaceComponentSourceFile {
                path: "src/value.js".to_string(),
                kind: SurfaceComponentSourceFileKind::Script,
        ...
        assert!(html.contains("sparo-surface-component/ui.js"));
        assert!(html.contains("sparo-surface-component/src/value.js"));
