//! Surface Component Runtime UI Kit.
//!
//! This is a small, stable runtime subset of the Web UI component library for
//! sandboxed Surface Components. It intentionally uses plain DOM helpers plus CSS class
//! contracts that mirror the component-library primitives, so Surface Components can use
...
/// through Surface Component host theme variables.
...
(function () {
  ...
  function mount(target, child) {
    ...
    if (!root) throw new Error('Surface Component UI Kit mount target not found');
  ...
  window.BitfunSurfaceComponentUI = ui;
  if (window.app) {
    window.app.ui = ui;
  }
})();
