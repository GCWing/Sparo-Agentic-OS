...
use crate::surface_component::manager::try_get_global_surface_component_manager;
use crate::surface_component::runtime_detect::DetectedRuntime;
use crate::surface_component::types::{
    SurfaceComponentRuntimeLog, SurfaceComponentRuntimeLogLevel,
};
...
fn parse_log_level(value: Option<&str>) -> SurfaceComponentRuntimeLogLevel {
    match value {
        Some("debug") => SurfaceComponentRuntimeLogLevel::Debug,
        Some("warn") => SurfaceComponentRuntimeLogLevel::Warn,
        Some("error") => SurfaceComponentRuntimeLogLevel::Error,
        _ => SurfaceComponentRuntimeLogLevel::Info,
    }
}

fn parse_worker_log_line(app_id: &str, line: &str) -> SurfaceComponentRuntimeLog {
    let parsed = serde_json::from_str::<Value>(line).ok();
    if let Some(value) = parsed.as_ref().and_then(Value::as_object) {
        return SurfaceComponentRuntimeLog {
    ...
    pub async fn spawn(
        ...
        tokio::spawn(async move {
            ...
            while let Ok(Some(line)) = lines.next_line().await {
                ...
                if let Some(event_name) = msg.get("event").and_then(Value::as_str) {
                    ...
                    let event_full_name =
                        format!("surface-component://worker-event:{}", stderr_app_id);
        ...
        tokio::spawn(async move {
            ...
            while let Ok(Some(line)) = lines.next_line().await {
                ...
                if let Some(manager) = try_get_global_surface_component_manager() {
