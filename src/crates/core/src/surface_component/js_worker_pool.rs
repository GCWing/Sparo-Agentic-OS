...
use crate::surface_component::js_worker::JsWorker;
use crate::surface_component::runtime_detect::{detect_runtime, DetectedRuntime};
use crate::surface_component::types::{NodePermissions, NpmDep};
...
impl JsWorkerPool {
    ...
    pub async fn get_or_spawn(
        ...
        let app_dir = self.path_manager.surface_component_dir(app_id);
        if !app_dir.exists() {
            return Err(BitFunError::NotFound(format!(
                "Surface Component dir not found: {}",
                app_id
            )));
        }
    ...
    pub async fn install_deps(
        ...
        let (cmd, args): (&str, &[&str]) = match self.runtime.kind {
            crate::surface_component::runtime_detect::RuntimeKind::Bun => {
                ("bun", &["install", "--production"][..])
            }
            crate::surface_component::runtime_detect::RuntimeKind::Node => {
