//! Host-side dispatch for SurfaceComponent framework primitives (`shell.exec`, `fs.*`, `os.info`,
//! `net.fetch`).
//!
//! Why this exists
//! ---------------
//! The original SurfaceComponent design routed every `app.*` call through a Bun/Node Worker
...
//! so SurfaceComponents that only use `app.shell.exec` / `app.fs.*` / `app.net.fetch` can run
//! with `permissions.node.enabled = false` and no JS Worker at all.
//!
//! Routing rules (must match `useSurfaceComponentBridge.ts`):
...
use crate::surface_component::types::SurfaceComponentPermissions;
...
///
/// `storage.*` is intentionally excluded: it is routed through `SurfaceComponentManager` from the
...
pub async fn dispatch_host(
    perms: &SurfaceComponentPermissions,
    app_id: &str,
    app_data_dir: &Path,
