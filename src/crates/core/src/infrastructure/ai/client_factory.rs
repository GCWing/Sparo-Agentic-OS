...
impl AIClientFactory {
    ...
    pub async fn initialize_global() -> CoreResult<()> {
        ...
        let config_service = get_global_config_service().await.map_err(|e| {
            CoreError::service(format!("Failed to get global config service: {}", e))
        })?;
        ...
        GLOBAL_AI_CLIENT_FACTORY.set(wrapper).map_err(|_| {
            CoreError::service("Failed to initialize global AIClientFactory".to_string())
        })?;
    ...
    pub async fn get_global() -> CoreResult<Arc<AIClientFactory>> {
        ...
        guard
            .as_ref()
            .ok_or_else(|| CoreError::service("Global AIClientFactory is None".to_string()))
            .map(Arc::clone)
    }
    ...
    pub async fn update_global(new_factory: Arc<AIClientFactory>) -> CoreResult<()> {
        let wrapper = GLOBAL_AI_CLIENT_FACTORY.get().ok_or_else(|| {
            CoreError::service("Global AIClientFactory not initialized".to_string())
        })?;
...
pub async fn get_global_ai_client_factory() -> CoreResult<Arc<AIClientFactory>> {
    AIClientFactory::get_global().await
}

pub async fn initialize_global_ai_client_factory() -> CoreResult<()> {
