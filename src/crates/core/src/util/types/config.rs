...
use log::warn;
pub use sparo_ai_adapters::types::resolve_request_url;
pub use sparo_ai_adapters::types::AIConfig;
...
impl TryFrom<AIModelConfig> for AIConfig {
    ...
    fn try_from(other: AIModelConfig) -> Result<Self, Self::Error> {
        ...
        Ok(AIConfig {
            ...
            top_p: other.top_p,
            reasoning_mode: other.reasoning_mode,
...
mod tests {
    ...
    fn resolves_gemini_request_url_bare_host() {
        assert_eq!(
            resolve_request_url("https://api.opensparo.com", "gemini", "gemini-2.5-pro"),
            "https://api.opensparo.com/v1beta/models/gemini-2.5-pro:streamGenerateContent?alt=sse"
        );
    }
    ...
    #[test]
    fn preserves_default_reasoning_mode() {
    ...
    #[test]
    fn preserves_enabled_reasoning_mode() {
        let mut model = base_model_config();
        model.reasoning_mode = ReasoningMode::Enabled;
