//! Persistent Node.js sidecar for Agent Component JavaScript runtime tools.
//!
//! Agent Component runtime tools used to spawn a fresh `node -e <bootstrap>` process on
...
/// Run a single Agent Component JS tool on the shared persistent Node sidecar and return
                ...
                loop {
                    ...
                    match reader.read_line(&mut line).await {
                        ...
                        Ok(_) => {
                            let trimmed = line.trim();
                            if !trimmed.is_empty() {
                                log::debug!("Agent Component JS runtime: {}", trimmed);
                            }
                        }
                    }
                }
...
async fn dispatch_response(pending: &PendingMap, message: Value) {
    ...
    let payload = if ok {
        ...
        Err(message
            ...
            .unwrap_or("Agent Component JS tool failed")
            .to_string())
    };
...
/// every Agent Component tool. The host capability surface mirrors the previous per-call
/// bootstrap exactly.
...
function buildContext(entry, permissions, workspaceRoot, readonly) {
  ...
  return {
    ...
    storage: {
      ...
      set: async (key, value) => {
        if (readonly) throw new Error('Readonly Agent Component JS tools cannot write storage');
        const file = path.join(appDir, '..', 'storage.json');
        let data = {};
