...
pub(crate) fn find_elements() -> &'static str {
    "(rootId, using, value) => window.__sparoWd.findElements(rootId, using, value)"
}
...
pub(crate) fn get_name() -> &'static str {
    "(id) => { const el = window.__sparoWd.getElement(id); return el ? String(el.tagName || '').toLowerCase() : ''; }"
}
...
pub(crate) fn is_enabled() -> &'static str {
    "(id) => { const el = window.__sparoWd.getElement(id); return !!el && !el.disabled; }"
}

pub(crate) fn click() -> &'static str {
    "(id) => { const el = window.__sparoWd.getElement(id); if (!el) { throw new Error('Element not found'); } window.__sparoWd.dispatchPointerClick(el, 0, false); return null; }"
}

pub(crate) fn clear() -> &'static str {
    "(id) => { const el = window.__sparoWd.getElement(id); if (!el) { throw new Error('Element not found'); } window.__sparoWd.clearElement(el); return null; }"
}

pub(crate) fn send_keys() -> &'static str {
    "(id, text) => { const el = window.__sparoWd.getElement(id); if (!el) { throw new Error('Element not found'); } window.__sparoWd.insertText(el, text); return null; }"
}

pub(crate) fn screenshot_metadata() -> &'static str {
    "(id) => { const el = window.__sparoWd.getElement(id); if (!el || !el.isConnected) { throw new Error('stale element reference'); } el.scrollIntoView({ block: 'center', inline: 'center' }); const rect = el.getBoundingClientRect(); return { x: rect.x, y: rect.y, width: rect.width, height: rect.height, devicePixelRatio: window.devicePixelRatio || 1 }; }"
}

pub(crate) fn get_shadow_root() -> &'static str {
    "(elementId) => window.__sparoWd.getShadowRoot(elementId)"
}

pub(crate) fn find_elements_from_shadow() -> &'static str {
    "(shadowId, using, value) => window.__sparoWd.findElementsFromShadow(shadowId, using, value)"
}

pub(crate) fn validate_frame_index() -> &'static str {
    "(index) => { if (!window.__sparoWd.validateFrameByIndex(index)) { throw new Error('Unable to locate frame'); } return true; }"
}

pub(crate) fn validate_frame_element() -> &'static str {
    "(elementId) => { if (!window.__sparoWd.validateFrameElement(elementId)) { throw new Error('Unable to locate frame'); } return true; }"
}
