...
(() => {{
  {helper}
  window.__sparoWd.run({request_id}, {script}, {args}, {async_mode}, {frame_context});
}})();
"#,
...
return (async () => {{
  {helper}
  const response = await window.__sparoWd.execute({script}, {args}, {async_mode}, {frame_context});
