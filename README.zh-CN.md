...
Sparo OS 按你的真实工作流，为你定制可长期使用、可持续演化的专属 Product App，而不是一次性的产物。这套应用体系由三层对象承载：

| 形态           | 定位         | 说明                                                                 |
| ------------ | ---------- | ------------------------------------------------------------------ |
| **Product App** | 用户可见应用入口 | 管理工作对象、交互界面、状态与运行反馈，具备持久身份，可持续演化、复用与回滚。 |
| **Agent Component** | AI 后端组件 | 把你的方法与流程沉淀成可复用的智能体能力，可作为 Product App 私有后端或共享组件。 |
| **Bridge Component** | 外部能力桥接组件 | 桥接 CLI、SDK、GUI、service、daemon 或 runtime，把存量软件接入同一套工作流。 |
...
Sparo OS 的 Product App 是一等公民，全部可在统一的应用中心中访问与管理。系统已内置覆盖三种形态的开箱即用应用：

**系统内置 Product App（自主执行型）**

| 应用         | 定位     | 说明                                                  |
| ---------- | ------ | --------------------------------------------------- |
| **Runno**  | OS 执行单元 | Sparo OS 的通用执行单元，灵活、高效、目标感强，适合处理各种类型任务。 |
| **App Builder** | 智能应用工坊 | 为你打造专属智能应用，理解需求或解析既有应用，重组功能、流程与方法，让应用更贴合你的工作方式。 |
| **Code**   | 高完成率 Code Agent | 以 BitFun Coder 交付，来自开源社区，全面处理实现、调试、重构、排障等软件开发任务。 |
| **Cowork** | 办公交付 | 面向办公交付的 Agent，擅长文档、表格、PPT 和资料整理，把日常工作推进成清晰成稿。 |
| **Design** | 产品与 UI 设计 | 把想法、需求和参考图转成原型、界面和视觉方案。 |

**Product App（可交互生成式）**

| 应用          | 定位     | 说明                                                       |
| ----------- | ------ | -------------------------------------------------------- |
| **PPT 工作台**   | 演示稿创作工作台 | 从主题到结构、页面和讲述节奏，生成可以继续编辑和展示的 PPT。 |
| **灵感画布**     | 思考画布 | 捕捉灵感、梳理脉络，把零散想法沉淀成草稿和下一步。 |

**桥接型 Product App / Bridge Component**

| 应用              | 定位         | 说明                                                |
| --------------- | ---------- | ------------------------------------------------- |
| **Cursor Bridge** | 桥接 Cursor SDK | 通过 Bridge Component 在 Sparo OS 中调度 Cursor SDK 智能体，针对本地工作区或 Cursor Cloud 仓库运行任务。 |
...
Sparo OS 内置了面向场景的 Tools 等能力，供你构建自己的 Product App；同时支持接入外部 Skill、MCP（含 MCP App）、自定义 Sub Agent 等作为 Kit，用于构建、调试并扩展你自己的 Product App。系统会随着你接入的能力不断生长。
