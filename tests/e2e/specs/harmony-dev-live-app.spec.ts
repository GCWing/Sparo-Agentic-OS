...
describe('HarmonyOS Dev Live App', () => {
  ...
  realHarmonyIt('opens as FlowChat plus a profile-owned HarmonyOS preview tab', async () => {
    await openLiveAppById('builtin-harmony-dev', harmonyFixturePath);

    const sessionScene = await $('[data-testid="session-scene"]');
    await sessionScene.waitForDisplayed({ timeout: 30000 });

    const previewPanel = await $('[data-testid="live-app-runner-panel"][data-app-id="builtin-harmony-dev"]');
