/**
 * L0 Bridge Component spec: verifies the desktop WebView can reach Bridge Component commands
...
describe('L0 Bridge Component Tests', () => {
  it('runs the Cursor SDK Bridge capability through Tauri IPC', async () => {
    const result = await browser.executeAsync<TauriBridgeSmokeResult>((done) => {
      (async () => {
        ...
        const bridgeComponents = await invoke<any[]>('list_bridge_components', {});
        const cursorBridge = bridgeComponents.find(app => app?.manifest?.id === 'cursor-sdk');

        if (!cursorBridge) {
          throw new Error('cursor-sdk Bridge Component was not found');
        }
        ...
        const run = await invoke<any>('run_bridge_component_action', {
          request: {
            appId: 'cursor-sdk',
        ...
        const storedRun = await invoke<any>('get_bridge_component_run', {
  ...
  it('registers the Cursor Agent tool from the Bridge Component bundle metadata', async () => {
    ...
    const result = await browser.executeAsync<AgentBridgeServiceResult>((workspacePath, done) => {
      (async () => {
        ...
        const bridgeComponents = await invoke<any[]>('list_bridge_components', {});
        const cursorBridge = bridgeComponents.find(app => app?.manifest?.id === 'cursor-sdk');
        const agentComponents = await invoke<any[]>('list_agent_components', { request: {} });
        const cursorAgent = agentComponents.find(app => app?.id === 'cursor-agent');
        const cursorSdkAgent = agentComponents.find(app => app?.id === 'cursor-sdk');
        const tools = await invoke<any[]>('get_all_tools_info', {});
        const cursorAgentTool = tools.find((tool: any) => tool?.name === 'bridgecomponent__cursor-sdk__CursorAgent');

        if (!cursorBridge) {
          throw new Error('cursor-sdk Bridge Component was not found');
        }
        if (!cursorSdkAgent) {
          throw new Error('cursor-sdk Bridge Component was not projected as an Agent Component entry');
        }
        if (!cursorAgentTool?.ui?.card) {
          throw new Error('cursor-sdk Bridge Component tool UI card metadata was not found');
        }
        ...
        const toolRun = await invoke<any>('execute_tool', {
          request: {
            toolName: 'bridgecomponent__cursor-sdk__CursorAgent',
            input: {
              prompt: 'E2E dry run through Bridge Component tool extension',
              mode: 'local',
              dryRun: true,
            },
            workspacePath: null,
        ...
        const failedRun = await invoke<any>('execute_tool', {
          ...
          bridgeComponent: {
            id: cursorBridge.manifest.id,
            allIds: bridgeComponents.map(app => app?.manifest?.id).filter(Boolean),
    ...
    expect((result as any).error).toBeUndefined();
    expect(result.bridgeComponent.id).toBe('cursor-sdk');
    expect(result.bridgeComponent.allIds).not.toContain('cursor-bridge');
    expect(result.bridgeComponent.toolNames).toEqual(['CursorAgent']);
    expect(result.agentComponent.cursorAgentExists).toBe(false);
    expect(result.agentComponent.cursorSdkExists).toBe(true);
    expect(result.agentComponent.cursorSdkTools).toEqual(['bridgecomponent__cursor-sdk__CursorAgent']);
    expect(result.session.agentType).toBe('cursor-sdk');
    expect(result.session.sessionId).toBeTruthy();
    expect(result.toolInfo.name).toBe('bridgecomponent__cursor-sdk__CursorAgent');
    expect(result.toolInfo.cardKind).toBe('appDefined');
    expect(result.toolInfo.cardTitle).toBe('Cursor Agent');
    expect(result.toolInfo.family).toBe('bridge-component');
