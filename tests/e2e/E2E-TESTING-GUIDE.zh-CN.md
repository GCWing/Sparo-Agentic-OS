...
#### Debug 模式（默认）
- **应用路径**: `target/debug/sparo-os.exe`
...
**核心原理**: E2E 只使用 `target/debug/sparo-os.exe`。如果 debug 二进制不存在，应直接失败，而不是回退到 `release`。
...
**症状**: `Application not found at target/debug/sparo-os.exe`
